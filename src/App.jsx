...
// Renders AI markdown text cleanly â no # symbols, proper bold, headers as styled text
function RenderMD({ text, style={} }) {
  ...
  while (i < lines.length) {
    const line = lines[i];
    // H1/H2/H3 â render as bold section header
    if (line.startsWith('### ')) {
    ...
    // Bullet points
      ...
      elements.push(<div key={i} style={{display:'flex',gap:8,marginBottom:3,fontSize:13,lineHeight:1.6}}><span style={{color:'var(--gold)',flexShrink:0,marginTop:1}}>â¢</span><span>{content}</span></div>);
...
// COMPLETE SUBJECT DATABASE â Years 9â12 VCE + IB
// âââââââââââââââââââââââââââââââââââââââââââââ
const ALL_SUBJECTS = {
  year9: {
    label: "Year 9",
  ...
  vce: {
    ...
    groups: {
      ...
      "Arts & Design": ["Visual Communication Design","Studio Arts","Art â Making & Exhibiting","Media","Music","Drama","Dance","Theatre Studies"],
  ...
  ib: {
    label: "IB Diploma",
    groups: {
      "Group 1 â Studies in Language": ["English A: Literature HL","English A: Language & Literature HL","English A: Literature SL","Self-Taught Language A"],
      "Group 2 â Language Acquisition": ["English B HL","French B HL","Japanese B SL","Spanish B SL","Chinese B SL","French ab initio","Spanish ab initio"],
      "Group 3 â Individuals & Societies": ["History HL","History SL","Economics HL","Economics SL","Geography HL","Geography SL","Psychology HL","Psychology SL","Global Politics HL","Business Management HL","Philosophy HL"],
      "Group 4 â Sciences": ["Biology HL","Biology SL","Chemistry HL","Chemistry SL","Physics HL","Physics SL","Environmental Systems SL","Computer Science HL","Sports Science SL"],
      "Group 5 â Mathematics": ["Maths: Analysis & Approaches HL","Maths: Analysis & Approaches SL","Maths: Applications & Interpretation HL","Maths: Applications & Interpretation SL"],
      "Group 6 â Arts": ["Visual Arts HL","Visual Arts SL","Music HL","Music SL","Theatre HL","Theatre SL","Film SL"],
...
const FUTURE_PATHS = [
  { id: "medicine", label: "Medicine / Health Sciences", icon: "ð¥", atar: "99+" },
  { id: "law", label: "Law", icon: "âï¸", atar: "95+" },
  { id: "engineering", label: "Engineering", icon: "âï¸", atar: "90+" },
  { id: "cs", label: "Computer Science / IT", icon: "ð»", atar: "85+" },
  { id: "business", label: "Business / Commerce", icon: "ð", atar: "80+" },
  { id: "arts_hum", label: "Arts / Humanities", icon: "ð¨", atar: "70+" },
  { id: "science", label: "Science / Research", icon: "ð¬", atar: "85+" },
  { id: "education", label: "Education / Teaching", icon: "ð", atar: "65+" },
  { id: "creative", label: "Creative Industries", icon: "ð¬", atar: "60+" },
  { id: "trade", label: "Trades / Vocational", icon: "ð§", atar: "N/A" },
  { id: "undecided", label: "Not Sure Yet", icon: "ð¤", atar: "â" },
];
...
/* Onboarding option â button element for reliable mobile touch */
.ob-opt{
...
function TypingIndicator() {
  return (
    <div className="msg ai">
      <div className="m-av m-ai-av">ð¤</div>
...
// SUPABASE CLIENT
// âââââââââââââââââââââââââââââââââââââââââââââ
// â ï¸  PASTE YOUR SUPABASE CREDENTIALS HERE
// Get them from: supabase.com â your project â Settings â API
...
// Tiny Supabase client â no npm needed, works in any browser/artifact
const supabase = (() => {
  const headers = {
  ...
  return {
    auth: {
      ...
      // Google OAuth â redirects browser to Google
      signInWithGoogle: () => {
...
function AuthScreen({ onAuth }) {
  ...
  const handleGoogle = () => {
    if (!configured) { setError("â ï¸ Add your Supabase credentials first â see the setup guide."); return; }
  ...
  const handleEmail = async () => {
    ...
    try {
      ...
      if (res.error_description || res.error) {
        ...
        // Sign-up returned user without token â email confirmation needed
        setError("â Check your email to confirm your account, then log in.");
      }
    } catch (e) {
    ...
    <div className="auth-wrap">
      <div className="auth-card fade-up">
        ...
        <div style={{fontSize:13,color:"var(--muted)",textAlign:"center",marginBottom:24,lineHeight:1.5}}>
          {mode === "login" ? "Welcome back â ready to study?" : "Join Victorian students getting better grades"}
        </div>
        ...
        {!configured && (
          <div style={{background:"var(--gold-light)",border:"1.5px solid var(--gold)",borderRadius:"var(--r)",padding:"10px 14px",marginBottom:16,fontSize:12,color:"var(--gold)",lineHeight:1.6}}>
            Demo mode â Supabase not connected yet.
          </div>
        )}

        {/* Google Sign In â proper button */}
        ...
        {error && <div style={{fontSize:12,color:error.startsWith("â")?"var(--success)":"var(--danger)",marginBottom:12,padding:"8px 12px",background:error.startsWith("â")?"var(--success-bg)":"var(--danger-bg)",borderRadius:"var(--r)",border:`1px solid ${error.startsWith("â")?"var(--success)":"var(--danger)"}`}}>{error}</div>}
...
function Onboarding({ user, onComplete }) {
  ...
  const step0 = (
    <>
      ...
      <div className="ob-grid">
        {[
          {id:"year9",label:"Year 9",icon:"ð",sub:"Junior Secondary"},
          {id:"year10",label:"Year 10",icon:"ð",sub:"Pre-senior year"},
          {id:"vce",label:"Year 11â12 VCE",icon:"ð",sub:"Victorian Certificate of Education"},
          {id:"ib",label:"Year 11â12 IB",icon:"ð",sub:"International Baccalaureate"},
        ].map(o=>(
      ...
      <button className="btn btn-p btn-full" style={{padding:13}} disabled={!data.yearLevel} onClick={next}>Continue â</button>
    </>
  );
    ...
    return (
      <>
            ...
            <div className="ob-subj-grid">
              {subjs.map(s=>(
                <button key={s} className={`ob-subj-btn${data.selectedSubjects.includes(s)?" sel":""}`}
                  ...
                  {data.selectedSubjects.includes(s)?"â ":""}{s}
                </button>
              ))}
            </div>
          </div>
        ...
        <div style={{display:"flex",gap:10,marginTop:8}}>
          <button className="btn btn-g btn-sm" onClick={back}>â Back</button>
          <button className="btn btn-p" style={{flex:1,justifyContent:"center",padding:12}} disabled={data.selectedSubjects.length===0} onClick={next}>
            Continue with {data.selectedSubjects.length} subject{data.selectedSubjects.length!==1?"s":""} â
          </button>
        </div>
      </>
    );
  ...
  // STEP 2 â Future path
  const step2 = (
    <>
      <div className="ob-step">Step 3 of {totalSteps}</div>
      <div className="ob-title">What's your goal? ð¯</div>
      ...
      <div className="ob-grid">
        {FUTURE_PATHS.map(p=>(
          <button key={p.id} type="button" className={`ob-opt${data.futurePath===p.id?" sel":""}`} onClick={()=>setData(d=>({...d,futurePath:p.id}))}>
            ...
            <div className="ob-opt-sub">{p.atar !== "â" ? `Typical ATAR: ${p.atar}` : "Portfolio / skills-based"}</div>
          </button>
        ))}
      </div>
      <div style={{display:"flex",gap:10}}>
        <button className="btn btn-g btn-sm" onClick={back}>â Back</button>
        <button className="btn btn-p" style={{flex:1,justifyContent:"center",padding:12}} disabled={!data.futurePath} onClick={next}>Continue â</button>
      </div>
    </>
  );

  // STEP 3 â Study hours
  const step3 = (
    <>
      <div className="ob-step">Step 4 of {totalSteps}</div>
      <div className="ob-title">How much can you study?</div>
      <div className="ob-sub">Be honest â we'll build a realistic schedule that avoids burnout.</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
        {[
          {id:"light",label:"Light",hours:"5â8 hrs/week",icon:"ð",desc:"Relaxed pace"},
          {id:"moderate",label:"Moderate",hours:"9â14 hrs/week",icon:"ð",desc:"Balanced approach"},
          {id:"intensive",label:"Intensive",hours:"15â20 hrs/week",icon:"ð¥",desc:"High performance mode"},
          {id:"extreme",label:"Exam Mode",hours:"21+ hrs/week",icon:"â¡",desc:"Full focus crunch"},
        ].map(o=>(
          <button key={o.id} type="button" className={`ob-opt${data.hoursPerWeek===o.id?" sel":""}`} onClick={()=>setData(d=>({...d,hoursPerWeek:o.id}))}>
            <div className="ob-opt-top"><span className="ob-opt-icon">{o.icon}</span><span className="ob-opt-label">{o.label}</span></div>
            <div className="ob-opt-sub">{o.hours} Â· {o.desc}</div>
          </button>
        ))}
      </div>
      <div style={{display:"flex",gap:10}}>
        <button className="btn btn-g btn-sm" onClick={back}>â Back</button>
        <button className="btn btn-p" style={{flex:1,justifyContent:"center",padding:12}} disabled={!data.hoursPerWeek} onClick={next}>Continue â</button>
      </div>
    </>
  );
  ...
  const step4 = (
    <>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:9,marginBottom:24}}>
        {[
          {id:"sac",label:"Preparing for upcoming SACs / IAs",icon:"ð"},
          {id:"exam",label:"Exam preparation & past papers",icon:"ð"},
          {id:"concepts",label:"Understanding difficult concepts",icon:"ð§ "},
          {id:"routine",label:"Building a consistent study routine",icon:"ð"},
          {id:"atar",label:"Maximising my ATAR / IB score",icon:"ð"},
          {id:"wellbeing",label:"Balancing study and wellbeing",icon:"ð±"},
        ].map(o=>(
      ...
      <div style={{display:"flex",gap:10}}>
        <button className="btn btn-g btn-sm" onClick={back}>â Back</button>
        <button className="btn btn-p" style={{flex:1,justifyContent:"center",padding:12}} disabled={!data.studyGoal} onClick={next}>Continue â</button>
      </div>
    </>
  );
  ...
  // STEP 5 â All set
  const goalText = {
    medicine:"Medicine or Health Sciences ð¥",law:"Law âï¸",engineering:"Engineering âï¸",
    cs:"Computer Science ð»",business:"Business / Commerce ð",arts_hum:"Arts & Humanities ð¨",
    science:"Science & Research ð¬",education:"Education ð",creative:"Creative Industries ð¬",
    trade:"Trades & Vocational ð§",undecided:"figuring out the right path ð¤"
  };
  const step5 = (
    <>
      ...
      <div className="ob-card" style={{marginBottom:10}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            ...
            {label:"Study Intensity",val:({light:"Light (5â8 hrs)",moderate:"Moderate (9â14 hrs)",intensive:"Intensive (15â20 hrs)",extreme:"Exam Mode (21+)"})[data.hoursPerWeek]||"â"},
          ].map(r=>(
  ...
  return (
    <div className="ob-wrap">
      <div className="ob-inner">
        ...
        <div className="ob-tag">Victorian Education Platform Â· Years 9â12</div>
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// GAME STATE â XP, streaks, mastery, quiz history
...
function useGameState(profile) {
  ...
  const load = () => {
    try {
      ...
      // Always ensure correct types â prevents K.map crashes
  ...
  // ATAR predictor â based on mastery scores weighted by subject count
  const predictATAR = () => {
    const s = load();
  ...
  return (
    <div className="content fade-up">
      <div style={{marginBottom:20}}>
        <div style={{fontWeight:900,fontSize:22}}>{greeting()}, {profile.userName?.split(" ")[0]} ð</div>
        <div style={{color:"var(--muted)",fontSize:13,marginTop:3}}>
          {isIB?"IB Diploma":ALL_SUBJECTS[profile.yearLevel]?.label} Â· {(Array.isArray(profile.selectedSubjects)?profile.selectedSubjects:[]).length} subjects Â· {today.toLocaleDateString("en-AU",{weekday:"long",month:"long",day:"numeric"})}
        </div>
      </div>
      ...
      <div className="g4" style={{marginBottom:18}}>
        ...
        <div className="stat">
          ...
          <div className="sl">Total XP Â· Level {state.level}</div>
        ...
        <div className="stat">
          <div className="sv" style={{color:"#FF6B6B"}}>ð¥ {state.streak}</div>
          <div className="sl">Day Streak</div>
          <div className="sc up">Best: {state.bestStreak} days</div>
        </div>
        <div className="stat">
          <div className="sv" style={{color:"#FFD700"}}>{avgAccuracy!=null?`${avgAccuracy}%`:"â"}</div>
      ...
      <div className="g2" style={{marginBottom:18}}>
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            ...
            <span className="cl" onClick={()=>setScreen("subjects")}>View all â</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            <div className="ch"><div className="ct">ð Upcoming Assessments</div><span className="cl" onClick={()=>setScreen("planner")}>+ Add â</span></div>
            <div style={{padding:"0 16px"}}>
              {upcoming.length === 0 ? (
                <div style={{padding:"16px 0",color:"var(--muted)",fontSize:13,textAlign:"center"}}>
                  No upcoming events â <span style={{color:"var(--accent)",cursor:"pointer"}} onClick={()=>setScreen("planner")}>add one in Planner</span>
                </div>
              ) : upcoming.map((u,i)=>{
          ...
          <div className="card">
            <div className="ch"><div className="ct">â¡ Quick Actions</div></div>
            <div className="cb" style={{display:"flex",flexDirection:"column",gap:7}}>
              <button className="btn btn-p btn-full" onClick={()=>setScreen("subjects")}>ð Study a Subject</button>
              <button className="btn btn-s btn-full" onClick={()=>setScreen("ai")}>â¨ Ask AI Tutor</button>
              <button className="btn btn-g btn-full" onClick={()=>setScreen("planner")}>ð Study Planner</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:18}}>
        <div className="ch">
          <div className="ct">ð Study Activity â Last 17 Weeks</div>
          <div style={{display:"flex",gap:5,alignItems:"center"}}>
            {[0,1,2,3,4].map(v=><div key={v} className={`hc h${v}`} style={{width:13,height:13,borderRadius:3}}/>)}
            <span style={{fontSize:10,color:"var(--muted)"}}>Less â More</span>
          </div>
        </div>
      ...
      <div className="g2">
        <div className="card">
          <div className="ch"><div className="ct">ð Achievements</div></div>
          <div className="cb">
            {[
              {icon:"ð¥",name:"Streak Starter",desc:"Study 7 days in a row",e:state.streak>=7},
              {icon:"â¡",name:"Quiz Master",desc:"Take 10 quizzes",e:(state.quizHistory?.length||0)>=10},
              {icon:"ð",name:"Century",desc:"Earn 100 XP",e:state.xp>=100},
              {icon:"ð¯",name:"Sharp Shooter",desc:"Score 100% on a quiz",e:state.quizHistory?.some(q=>q.pct===100)},
              {icon:"ð",name:"High Achiever",desc:"Reach 1000 XP",e:state.xp>=1000},
              {icon:"ð",name:"Dedicated",desc:"Study 30 days in a row",e:state.streak>=30},
            ].map((a,i)=>(
              <div key={i} className={`ach${a.e?" earned":""}`}>
                ...
                <div style={{marginLeft:"auto",fontSize:16}}>{a.e?"â":"ð"}</div>
              </div>
            ))}
          </div>
        </div>
        <div className="card">
          <div className="ch"><div className="ct">ð Recent Quiz Results</div><span className="cl" onClick={()=>setScreen("analytics")}>Analytics â</span></div>
          <div className="cb">
            {state.quizHistory?.length === 0 || !state.quizHistory ? (
              <div style={{color:"var(--muted)",fontSize:13,textAlign:"center",padding:"12px 0"}}>No quizzes yet â <span style={{color:"var(--accent)",cursor:"pointer"}} onClick={()=>setScreen("quiz")}>start one!</span></div>
              ...
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<4?"1px solid var(--border)":"none"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:10,color:"var(--muted)"}}>{q.score}/{q.total} correct Â· {new Date(q.date).toLocaleDateString("en-AU",{month:"short",day:"numeric"})}</div>
                </div>
              </div>
            ))}
          </div>
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// GEMINI HELPER â calls YOUR Vercel backend
...
const VCAA_CURRICULUM = {
  ...
  "Maths Methods": {
    ...
    areas: [
      ...
      { name:"Calculus â Differentiation", dotPoints:["Limits and continuity","Differentiation by first principles","Power rule, product rule, quotient rule, chain rule","Derivatives of sin, cos, tan, e^x, log_e x","Applications: rates of change, gradient, stationary points","Optimisation problems"] },
      { name:"Calculus â Integration", dotPoints:["Antidifferentiation and indefinite integrals","Definite integrals and area under a curve","Fundamental theorem of calculus","Integration of polynomial, exponential, trigonometric functions","Applications: total change, area between curves"] },
  ...
  "Chemistry": {
    ...
    areas: [
      ...
      { name:"Electrochemistry", dotPoints:["Oxidation and reduction â electron transfer","Galvanic cells: anode, cathode, salt bridge, cell potential","Standard electrode potentials and EMF calculations","Electrolytic cells and electrolysis","Fuel cells and rechargeable batteries","Faraday's laws of electrolysis"] },
      { name:"Energetics & Thermodynamics", dotPoints:["Enthalpy changes in reactions (ÎH)","Hess's Law and enthalpy cycles","Calorimetry calculations","Bond energies","Entropy (ÎS) and Gibbs free energy (ÎG = ÎH â TÎS)","Spontaneity of reactions"] },
  ...
  "Physics": {
    units:"Units 3 & 4", assessmentType:"SAC + end-of-year exam",
    areas: [
      { name:"Motion & Forces", dotPoints:["Newton's laws of motion in 2D","Momentum and impulse (FÂ·Ît = Îp)","Work, energy, power","Conservation of energy and momentum","Circular motion: centripetal acceleration and force","Gravitational fields and orbital motion"] },
      { name:"Electricity & Magnetism", dotPoints:["Electric fields and force between charges","Electric potential and potential difference","DC circuits: Ohm's Law, series/parallel, Kirchhoff's laws","Magnetic fields and forces on current-carrying conductors","Electromagnetic induction â Faraday's and Lenz's Laws","Transformers and AC generators"] },
  ...
  "Psychology": {
    ...
    areas: [
      ...
      { name:"States of Consciousness", dotPoints:["Normal waking consciousness and altered states","Sleep: NREM stages, REM, sleep cycles","Functions of sleep â restoration and memory consolidation","Sleep disorders: insomnia, sleep apnoea, narcolepsy","Effects of sleep deprivation","Melatonin and circadian rhythms"] },
  ...
  "Legal Studies": {
    ...
    areas: [
      ...
      { name:"Law Reform & Justice", dotPoints:["Reasons for law reform â changing values, new technology","Role of parliament and courts in law reform","Institutions: Law Reform Commission, Royal Commissions","Evaluating the legal system: fairness, equality, access","Australian Constitution and division of powers"] },
    ]
  },
  "History: Revolutions": {
    units:"Units 3 & 4", assessmentType:"SAC + end-of-year exam",
  ...
  "Economics": {
    units:"Units 3 & 4", assessmentType:"SAC + end-of-year exam",
    areas: [
      { name:"Aggregate Demand & Supply", dotPoints:["Components of GDP: C + I + G + (XâM)","Aggregate demand shifts and multiplier effect","Aggregate supply: short-run and long-run","Equilibrium output and price level","Economic growth and business cycles"] },
  ...
  // ââ HEALTH & PE ââ
  "Health & PE": {
    units:"Years 9â10 + VCE Units 1â4", assessmentType:"Practical + written assessment",
    areas: [
      { name:"Health & Wellbeing Concepts", dotPoints:["Dimensions of health: physical, social, emotional, mental, spiritual","Health as a dynamic concept â biopsychosocial model","Factors influencing health: biological, sociocultural, physical environment","Health status of Australians â key statistics and indicators","Burden of disease: DALY, YLL, YLD"] },
      ...
      { name:"Sport Psychology & Performance", dotPoints:["Arousal, anxiety and performance â inverted U hypothesis","Goal setting: SMART goals","Motivation: intrinsic and extrinsic","Mental skills: imagery, self-talk, relaxation techniques","Team dynamics: cohesion, leadership, communication"] },
    ]
  },
  ...
  "Physical Education": {
    ...
    areas: [
      ...
      { name:"Energy Systems", dotPoints:["ATP structure and resynthesis","ATP-PC (phosphocreatine) system â intensity, duration, recovery","Lactic acid (glycolytic) system â lactate threshold","Aerobic (oxidative) system â glycolysis, Krebs cycle, ETC","Interplay of energy systems during exercise","Fatigue mechanisms and recovery"] },
  ...
  "General Maths": {
    units:"Units 1â4", assessmentType:"SAC + end-of-year exam (CAS allowed)",
    areas: [
  ...
  "Foundation Maths": {
    units:"Units 1â4", assessmentType:"SAC (no end-of-year exam)",
    areas: [
  ...
  "Environmental Science": {
    ...
    areas: [
      ...
      { name:"Human Impact & Sustainability", dotPoints:["Ecological footprint and carrying capacity","Pollution: air, water, soil â sources and impacts","Waste management: reduce, reuse, recycle","Sustainable development principles","Environmental impact assessment"] },
    ]
  },
  ...
  "Geography": {
    ...
    areas: [
      { name:"Changing the Land", dotPoints:["Land use change: agriculture, urbanisation, deforestation","Environmental impacts: erosion, soil degradation, loss of biodiversity","Natural hazards: bushfire, flood, drought â causes and impacts","Land management strategies","Case studies: Australian and global examples"] },
  ...
  // ââ YEAR 9 & 10 SUBJECTS ââ
  "Mathematics": {
    units:"Year 9â10", assessmentType:"School-based assessments + exams",
    areas: [
  ...
  "Science": {
    units:"Year 9â10", assessmentType:"School-based assessments + practical investigations",
    areas: [
      ...
      { name:"Physics", dotPoints:["Motion: distance, displacement, speed, velocity, acceleration","Forces: Newton's Laws, friction, gravity, normal force","Energy: kinetic, potential, conservation, work, power","Waves: sound and light â properties, reflection, refraction","Electricity: circuits, current, voltage, resistance, Ohm's Law"] },
  ...
  "English (Year 9-10)": {
    units:"Years 9â10", assessmentType:"School-based assessments",
    areas: [
      { name:"Reading & Responding to Texts", dotPoints:["Analyse how authors construct meaning using language and literary features","Identify and discuss the ideas, themes and values in texts","Develop and justify a written interpretation of a text","Use metalanguage: symbolism, imagery, tone, structure, characterisation","Compare and contrast texts in a written response"] },
      { name:"Creating & Presenting", dotPoints:["Create written texts for a specific audience and purpose","Adapt language and form to context","Demonstrate understanding of persuasive and imaginative writing techniques","Use rhetorical devices effectively","Craft arguments supported by evidence"] },
      { name:"Analysing Argument", dotPoints:["Identify the contention, arguments and persuasive language in texts","Analyse how language and visuals are used to persuade","Evaluate the effectiveness of arguments","Discuss the point of view and context of the author","Write a formal analytical response to argument"] },
  ...
  "History (Year 9-10)": {
    units:"Year 9â10", assessmentType:"School-based assessments + essays",
    areas: [
  ...
  "Geography (Year 9-10)": {
    units:"Year 9â10", assessmentType:"School-based assessments + fieldwork",
    areas: [
      { name:"Biomes & Food Security (Year 9)", dotPoints:["World biomes: rainforest, desert, grassland, tundra â characteristics","Factors affecting food production: climate, soil, water, technology","Food security vs food insecurity â global distribution","Impacts of changing environments on food production","Strategies to improve food security globally"] },
  ...
  "Maths: Analysis & Approaches SL": {
    units:"IB SL â 2 years", assessmentType:"IA (20%) + Paper 1 & 2 exams (80%)",
    areas: [
  ...
  "Digital Technologies": {
    units:"Year 9â10", assessmentType:"School-based project assessments",
    areas: [
  ...
  "Commerce": {
    ...
    areas: [
      ...
      { name:"Personal Finance", dotPoints:["Income: wages, salary, commission, government payments","Budgeting: income vs expenses, saving strategies","Banking: types of accounts, interest rates","Credit: credit cards, loans, mortgages â advantages and risks","Superannuation and long-term financial planning"] },
  ...
  "Biology HL": {
    units:"IB HL â 2 years", assessmentType:"IA (20%) + Paper 1, 2, 3 exams (80%)",
    areas: [
      { name:"Cell Biology", dotPoints:["Cell theory, ultrastructure of cells (prokaryotic and eukaryotic)","Membrane structure â fluid mosaic model","Transport: diffusion, osmosis, active transport, endocytosis","Cell division: mitosis (PMAT), cytokinesis","Stem cells: types, therapeutic uses, ethical issues"] },
  ...
  "Chemistry HL": {
    ...
    areas: [
      ...
      { name:"Energetics & Kinetics", dotPoints:["Enthalpy changes: standard enthalpies, Hess's Law, Born-Haber cycles (HL)","Entropy and Gibbs free energy (ÎG = ÎH â TÎS)","Reaction rates: collision theory, activation energy, Arrhenius equation (HL)","Catalysts: homogeneous and heterogeneous","Maxwell-Boltzmann distribution"] },
      { name:"Equilibrium & Acid-Base", dotPoints:["Equilibrium constant: Kc, Kp expressions and calculations","Le Chatelier's Principle with industrial applications","BrÃ¸nsted-Lowry and Lewis acid-base theories","pH calculations: strong/weak acids and bases, buffers","Titration curves and indicators"] },
  ...
  "History HL": {
    units:"IB HL â 2 years", assessmentType:"IA (20%) + Paper 1, 2, 3 exams (80%)",
    areas: [
      { name:"Paper 1 â Prescribed Subjects", dotPoints:["Source analysis: provenance, origin, purpose, limitations","Cross-referencing sources","Historiography: how historians interpret evidence differently","Writing evidence-based historical arguments","Common prescribed subjects: Rights (civil rights, apartheid), Conflict (WWI, WWII)"] },
      { name:"Paper 2 â World History Topics", dotPoints:["Causes, practices and effects of war","Democratic states: challenges and responses","Origins and development of authoritarian and single-party states","Independence movements in Asia and Africa","Cold War: superpower tensions and proxy conflicts"] },
      { name:"Paper 3 â Regional Option (HL)", dotPoints:["In-depth study of a specific region (e.g. Europe, Asia-Pacific, Americas)","Comparative historical analysis across multiple countries","Long-term and short-term causes of historical events","Impact on different social groups within the region","Evaluating historical significance and legacy"] },
  ...
  "Japanese SL": {
    ...
    areas: [
      ...
      { name:"Speaking â Oral Assessment", dotPoints:["Participate in a conversation on a prepared topic","Discuss personal experiences, opinions and ideas","Use appropriate language for formal and informal situations","Demonstrate range of vocabulary and grammatical structures","Respond to unprepared questions during oral examination"] },
      { name:"Writing & Grammar", dotPoints:["Write in hiragana, katakana and approximately 200 kanji","Grammatical structures: ã¦-form, ã-form, potential, passive, causative, conditional","Particles: ã¯ãããããã«ãã§ãã¨ããããã¾ã§","Sentence endings: ã§ã/ã¾ã and plain form","Writing for different purposes: letters, emails, essays, narratives"] },
    ]
  },
  ...
  "French SL": {
    ...
    areas: [
      ...
      { name:"Grammar Essentials", dotPoints:["Verb tenses: prÃ©sent, passÃ© composÃ©, imparfait, futur, conditionnel, subjonctif","Pronoun use: direct, indirect, reflexive, relative pronouns","Adjective agreement and placement","Negation structures: ne...pas, ne...jamais, ne...rien","Subjunctive mood for doubt, emotion, necessity"] },
    ]
  },
  ...
  "Chinese SL": {
    ...
    areas: [
      ...
      { name:"Writing", dotPoints:["Write in simplified Chinese characters accurately","Grammatical structures: aspect markers (äºãè¿ãç), resultative complements, topic-comment structure","Sentence patterns: æ-sentences, è¢«-passive, æ¯...ç construction","Write for different purposes: letters, emails, narratives, explanations","Demonstrate understanding of Chinese cultural conventions in writing"] },
    ]
  },
  ...
  "Latin": {
    ...
    areas: [
      ...
      { name:"Grammar & Syntax", dotPoints:["Noun declensions 1â5: all cases and their uses","Verb conjugations: all tenses active and passive (indicative and subjunctive)","Participles: present, perfect, future active and passive","Indirect statement with accusative + infinitive","Subordinate clauses: purpose, result, indirect command, conditional"] },
  ...
  "Food Studies": {
    ...
    areas: [
      { name:"Food Science & Nutrition", dotPoints:["Macronutrients: carbohydrates, proteins, fats â structure, function, food sources","Micronutrients: vitamins and minerals â roles and deficiency diseases","Australian Dietary Guidelines and Healthy Eating Plate","Digestion and absorption of nutrients","Food as a source of energy: kilojoules and metabolic rate"] },
  ...
  // ââ VCE HUMANITIES ââ
  "Philosophy": {
    units:"VCE Units 3 & 4", assessmentType:"SAC + end-of-year exam",
    areas: [
      { name:"Epistemology â Knowledge & Belief", dotPoints:["What is knowledge? Justified True Belief (JTB)","Gettier problems and the challenge to JTB","Sources of knowledge: reason (rationalism) vs experience (empiricism)","Scepticism: Descartes' Meditations, brain-in-a-vat","A priori vs a posteriori knowledge"] },
      { name:"Metaphysics â Mind & Reality", dotPoints:["The mind-body problem: dualism (Descartes), physicalism, functionalism","Personal identity: psychological continuity, physical continuity","Free will and determinism: compatibilism, hard determinism, libertarianism","The nature of time: A-theory vs B-theory","Philosophy of religion: arguments for and against God's existence"] },
  ...
  "Sociology": {
    ...
    areas: [
      ...
      { name:"Sociological Theory", dotPoints:["Functionalism: Durkheim, Parsons â society as a system","Conflict theory: Marx, Weber â power, class and social change","Symbolic interactionism: Mead, Goffman â meaning and everyday life","Feminist sociology: patriarchy, gender inequality, intersectionality","Postmodernism and its critique of grand narratives"] },
  ...
  "VET Hospitality": {
    ...
    areas: [
      { name:"Food Preparation & Cookery", dotPoints:["Safe food handling and hygiene practices (HACCP)","Knife skills and basic cookery techniques: sautÃ©, braise, grill, deep fry","Preparing stocks, sauces and soups","Menu planning and recipe costing","Dietary requirements: allergies, intolerances, cultural and religious needs"] },
  ...
  // ââ IB SL/HL ADDITIONS ââ
  "Biology SL": {
    units:"IB SL â 2 years", assessmentType:"IA (20%) + Paper 1 & 2 exams (80%)",
    areas: [
      { name:"Cell Biology", dotPoints:["Cell theory and ultrastructure: prokaryotic and eukaryotic cells","Membrane structure and transport: diffusion, osmosis, active transport","Cell division: mitosis â interphase, PMAT, cytokinesis","Origin of cells: spontaneous generation vs biogenesis","Stem cells: types and therapeutic potential"] },
  ...
  "Chemistry SL": {
    ...
    areas: [
      ...
      { name:"Equilibrium & Acids/Bases", dotPoints:["Dynamic equilibrium and Le Chatelier's Principle","Acid-base theories: BrÃ¸nsted-Lowry","pH scale and calculations for strong acids/bases","Neutralisation and titration","Buffer solutions and their importance"] },
  ...
  "History SL": {
    units:"IB SL â 2 years", assessmentType:"IA (20%) + Paper 1 & 2 exams (80%)",
    areas: [
      { name:"Paper 1 â Source Analysis", dotPoints:["Evaluating sources: origin, purpose, value, limitation","Cross-referencing and comparing sources","Identifying perspective, bias and propaganda","Using sources to support historical arguments","Common Paper 1 themes: rights, conflict, leadership"] },
      { name:"Paper 2 â World History", dotPoints:["Causes, practices and effects of wars (20th century)","Democratic states: challenges and responses (1918-2000)","Authoritarian states: rise and nature of rule","Cold War: causes, development, effects","Independence movements and decolonisation"] },
      ...
      { name:"Internal Assessment", dotPoints:["Choosing a historical investigation question","Section 1: Identification and evaluation of two sources","Section 2: Investigation â using evidence to answer the question","Section 3: Reflection on the nature of history as a discipline","Academic writing: referencing, citation, bibliography"] },
    ]
  },

  "Global Politics HL": {
    units:"IB HL â 2 years", assessmentType:"IA + Engagement Activity (20%) + Paper 1 & 2 (80%)",
    areas: [
  ...
  "Business Management HL": {
    units:"IB HL â 2 years", assessmentType:"IA (20%) + Paper 1 & 2 exams (80%)",
    areas: [
  ...
  "Computer Science HL": {
    units:"IB HL â 2 years", assessmentType:"IA + case study (20%) + Paper 1, 2, 3 (80%)",
    areas: [
  ...
  "Visual Arts HL": {
    units:"IB HL â 2 years", assessmentType:"Exhibition + Comparative Study + Process Portfolio (100%)",
    areas: [
  ...
  "Theatre HL": {
    units:"IB HL â 2 years", assessmentType:"Research Presentation + Collaborative Project + Solo Theatre Piece (100%)",
    areas: [
      ...
      { name:"HL Extension â Independent Portfolio", dotPoints:["Writing critical analysis of live or recorded theatre works","Examining how theatre creates meaning through all production elements","Researching and responding to theatre from a range of cultural contexts","Developing as a reflective theatre maker and analyst","Extended documentation of creative and analytical processes"] },
    ]
  },

  "Theory of Knowledge (TOK)": {
    units:"IB Core â 2 years", assessmentType:"TOK Exhibition (33%) + TOK Essay (67%)",
    areas: [
      { name:"Core Theme â Knowledge & the Knower", dotPoints:["What does it mean to know something?","Personal knowledge vs shared knowledge","The relationship between knowledge, belief and truth","Knower's perspective: bias, assumptions, prior knowledge","The role of emotions, reason, language and sense perception in knowing"] },
  ...
  "Extended Essay (EE)": {
    units:"IB Core â 2 years", assessmentType:"Written essay (75%) + Viva Voce (25%)",
    areas: [
  ...
  "CAS (Creativity, Activity, Service)": {
    units:"IB Core â 2 years", assessmentType:"Ongoing portfolio + reflections (pass/fail)",
    areas: [
  ...
  // ââ YEAR 9-10 ELECTIVES ââ
  "Drama (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Practical performance + written response",
    areas: [
  ...
  "Music (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Performance + musicology tasks + aural tests",
    areas: [
  ...
  "Visual Arts (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Folio + resolved artwork + written response",
    areas: [
  ...
  "Food Technology (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Practical tasks + written assessments",
    areas: [
      { name:"Food Science & Nutrition", dotPoints:["Macronutrients: carbohydrates, proteins, fats â functions and sources","Micronutrients: vitamins and minerals â key roles","Australian Dietary Guidelines for adolescents","Reading and interpreting food labels: nutrition information panel, ingredients","Special dietary needs: vegetarian, vegan, gluten-free, lactose-intolerant, religious requirements"] },
      { name:"Food Safety & Hygiene", dotPoints:["Personal hygiene practices in food preparation","Cross-contamination: causes and prevention","Temperature control: danger zone (5Â°Câ60Â°C), safe cooking temperatures","Food storage: FIFO, correct temperatures, containers","Australian food safety laws and standards"] },
  ...
  "Design & Technology (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Design folio + practical project",
    areas: [
  ...
  "Media Studies (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Media production + analytical tasks",
    areas: [
  ...
  "Japanese (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Listening, reading, writing and speaking tasks",
    areas: [
      ...
      { name:"Reading & Writing", dotPoints:["Reading hiragana, katakana and approximately 100 kanji","Writing simple sentences and short paragraphs in Japanese script","Using basic grammatical structures: ã¯ãããããã« particles","Verb forms: dictionary form, ã¾ã form, ã¦-form","Reading simple texts: menus, signs, emails, short stories"] },
      { name:"Grammar & Vocabulary", dotPoints:["Sentence structure: Subject-Object-Verb (SOV)","Adjective types: ã-adjectives and ãª-adjectives","Verb conjugation: present/future, past, negative, ã¦-form","Question words: ãªã«ãã©ãããã¤ãã ããã©ããã¦","Common expressions: ãããã¨ãããã¿ã¾ããããããã¾ãã"] },
  ...
  "French (Year 9-10)": {
    ...
    areas: [
      ...
      { name:"Grammar Essentials", dotPoints:["Verb tenses: prÃ©sent, passÃ© composÃ©, imparfait, futur proche, futur simple","Gender and agreement: adjectives, articles, past participles","Pronoun use: personal, direct, indirect, reflexive","Negation: ne...pas, ne...jamais, ne...rien, ne...plus","Question forms: inversion, est-ce que, interrogative pronouns"] },
      { name:"French Culture", dotPoints:["Francophone world: France, QuÃ©bec, Africa, Pacific French territories","French customs, food and daily life","French history and its global influence","Contemporary French culture: cinema, music, sport, fashion","Comparing French and Australian social norms and values"] },
    ]
  },

  "Digital Technologies (Year 9-10)": {
    units:"Year 9â10", assessmentType:"Projects + written tasks",
    areas: [
...
function getCurriculumContext(subject, topic, userNotes, yearLevel) {
  ...
  if (subjData) {
    ...
    if (topic) {
      ...
      if (area) {
        ...
        area.dotPoints.forEach(dp => { context += `â¢ ${dp}\n`; });
      } else {
        context += `\nCurrent Topic: ${topic}\n`;
      ...
      subjData.areas.forEach(a => {
        context += `\n${a.name}:\n`;
        a.dotPoints.forEach(dp => { context += `â¢ ${dp}\n`; });
      });
    }
  } else {
  ...
  if (userNotes?.trim()) {
    context += `\nSTUDENT'S OWN NOTES/TEXTBOOK (USE THIS AS PRIMARY SOURCE â prioritise over general curriculum):\n${userNotes}\n`;
  }
  return context;
}

// âââââââââââââââââââââââââââââââââââââââââââââ
// CLEAN MATH â converts LaTeX to readable symbols
...
function convertLatex(math) {
  return math
    ...
    .replace(/\\neq/g, 'â ')
    .replace(/\\approx/g, 'â')
    .replace(/\\therefore/g, 'â´')
    .replace(/\\because/g, 'âµ')
    .replace(/\\in/g, 'â')
    .replace(/\\notin/g, 'â')
    .replace(/\\subset/g, 'â')
    .replace(/\\pi/g, 'Ï')
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// SUBTOPICS DATABASE â structured content per topic area
// âââââââââââââââââââââââââââââââââââââââââââââ
const SUBTOPICS = {
  "Number & Algebra": [
    {id:"surds",emoji:"â",title:"Surds & Irrational Numbers",summary:"Numbers that can't be expressed as exact fractions, left in root form.",keyFacts:["â2, â3, â5 are irrational â they go on forever","Simplify: â72 = â(36Ã2) = 6â2","Multiply: âa Ã âb = â(ab)","Divide: âa Ã· âb = â(a/b)","Can't add unlike surds: â2 + â3 â  â5"],formula:"â(ab) = âa Ã âb"},
    {id:"sci-notation",emoji:"Ã10",title:"Scientific Notation",summary:"Writing very large or small numbers as a Ã 10â¿ where 1 â¤ a < 10.",keyFacts:["4,500,000 = 4.5 Ã 10â¶","0.00032 = 3.2 Ã 10â»â´","Multiply: add the powers of 10","Divide: subtract the powers of 10","Used in science for astronomical/microscopic values"],formula:"a Ã 10â¿ where 1 â¤ a < 10"},
    {id:"expanding",emoji:"()",title:"Expanding Brackets",summary:"Multiply every term inside the brackets by the term outside.",keyFacts:["a(b+c) = ab + ac","(a+b)(c+d) = ac + ad + bc + bd","(a+b)Â² = aÂ² + 2ab + bÂ²","(a-b)Â² = aÂ² - 2ab + bÂ²","(a+b)(a-b) = aÂ² - bÂ²  (difference of squares)"],formula:"(a+b)Â² = aÂ² + 2ab + bÂ²"},
    {id:"factorising",emoji:"Ã·",title:"Factorising",summary:"The reverse of expanding â find what multiplies to give an expression.",keyFacts:["Find HCF first: 6x + 9 = 3(2x + 3)","Difference of squares: aÂ²-bÂ² = (a+b)(a-b)","Trinomials: xÂ²+5x+6 = (x+2)(x+3)","Find two numbers that multiply to c and add to b","Always check by expanding back"],formula:"aÂ²-bÂ² = (a+b)(a-b)"},
    {id:"indices",emoji:"aâ¿",title:"Index Laws",summary:"Rules for working with powers and exponents.",keyFacts:["aáµ Ã aâ¿ = aáµâºâ¿  (multiply: add powers)","aáµ Ã· aâ¿ = aáµâ»â¿  (divide: subtract powers)","(aáµ)â¿ = aáµâ¿  (power of power: multiply)","aâ° = 1  (anything to power 0)","aâ»â¿ = 1/aâ¿  (negative power = reciprocal)"],formula:"aáµ Ã aâ¿ = aáµâºâ¿"},
  ],
  "Measurement & Geometry": [
    {id:"pythagoras",emoji:"ð",title:"Pythagoras' Theorem",summary:"In a right-angled triangle, the square of the hypotenuse equals the sum of squares of the other two sides.",keyFacts:["aÂ² + bÂ² = cÂ²  (c is always the hypotenuse)","Hypotenuse is opposite the right angle â always the longest side","Finding hypotenuse: c = â(aÂ² + bÂ²)","Finding a shorter side: a = â(cÂ² - bÂ²)","Pythagorean triples: 3,4,5 and 5,12,13 and 8,15,17"],formula:"aÂ² + bÂ² = cÂ²"},
    {id:"trigonometry",emoji:"ð",title:"Trigonometry â SOH CAH TOA",summary:"Using ratios of sides in right-angled triangles to find unknown sides and angles.",keyFacts:["SOH: sin Î¸ = Opposite / Hypotenuse","CAH: cos Î¸ = Adjacent / Hypotenuse","TOA: tan Î¸ = Opposite / Adjacent","To find an angle: use inverse sinâ»Â¹, cosâ»Â¹, tanâ»Â¹","Label sides relative to the angle you're working with"],formula:"sin Î¸ = O/H   cos Î¸ = A/H   tan Î¸ = O/A"},
    {id:"area",emoji:"â¬",title:"Area & Perimeter",summary:"Area is the space inside a 2D shape. Perimeter is the distance around it.",keyFacts:["Rectangle: A = l Ã w, P = 2(l+w)","Triangle: A = Â½ Ã b Ã h","Circle: A = ÏrÂ², Circumference = 2Ïr","Trapezium: A = Â½(a+b)h","Parallelogram: A = b Ã h"],formula:"Triangle: A = Â½bh   Circle: A = ÏrÂ²"},
    {id:"volume",emoji:"ð¦",title:"Volume & Surface Area",summary:"Volume is the 3D space inside a solid. Surface area is the total of all faces.",keyFacts:["Rectangular prism: V = lwh","Cylinder: V = ÏrÂ²h","Triangular prism: V = Â½bhl","Cylinder SA = 2ÏrÂ² + 2Ïrh","Sphere: V = 4/3 ÏrÂ³, SA = 4ÏrÂ²"],formula:"Cylinder: V = ÏrÂ²h   Prism: V = base area Ã height"},
    {id:"similarity",emoji:"ð",title:"Similar Figures & Scale Factors",summary:"Shapes are similar if they have the same angles and proportional sides.",keyFacts:["Matching angles are equal","Corresponding sides are in the same ratio","Scale factor k: if sides Ã k, area Ã kÂ², volume Ã kÂ³","Find scale factor: divide matching sides","Use ratios to find unknown sides"],formula:"scale factor k: area ratio = kÂ²"},
    {id:"circle-geo",emoji:"â­",title:"Circle Geometry",summary:"Properties and theorems relating to circles, chords, tangents and angles.",keyFacts:["Angle in semicircle = 90Â°","Angle at centre = 2 Ã angle at circumference","Angles in same segment are equal","Tangent is perpendicular to radius at point of contact","Chord from centre bisects the chord at 90Â°"]},
  ],
  "Statistics & Probability": [
    {id:"data-types",emoji:"ð",title:"Data Types & Displays",summary:"Data is either categorical (types) or numerical (counts/measurements).",keyFacts:["Categorical: colour, gender, type â no numbers","Numerical discrete: countable whole numbers (goals scored)","Numerical continuous: measurements (height, time)","Dot plots and stem-and-leaf: for small data sets","Box plots: show median, quartiles, outliers"],},
    {id:"summary-stats",emoji:"ð",title:"Summary Statistics",summary:"Single numbers that describe the centre and spread of a data set.",keyFacts:["Mean: sum Ã· count (affected by outliers)","Median: middle value when ordered (resistant to outliers)","Mode: most frequent value","Range: max â min","IQR: Q3 â Q1 (middle 50% of data)"],formula:"Mean = Î£x/n   IQR = Q3 - Q1"},
    {id:"probability",emoji:"ð²",title:"Probability",summary:"The likelihood of an event occurring, expressed as a number from 0 to 1.",keyFacts:["P(event) = favourable outcomes Ã· total outcomes","P(certain) = 1, P(impossible) = 0","Complement: P(not A) = 1 - P(A)","P(A and B) = P(A) Ã P(B) for independent events","P(A or B) = P(A) + P(B) - P(A and B)"],formula:"P(A) = n(A) / n(S)"},
  ],
  "Patterns & Algebra": [
    {id:"linear-equations",emoji:"âï¸",title:"Linear Equations",summary:"Equations where the highest power is 1. Solve by doing the same to both sides.",keyFacts:["Isolate the variable step by step","Expand brackets first if needed","Collect like terms on each side","Move variables to one side, numbers to the other","Check: substitute your answer back in"]},
    {id:"graphing",emoji:"ð",title:"Graphing Linear Relationships",summary:"Linear relationships form straight lines on a number plane.",keyFacts:["y = mx + b (gradient-intercept form)","m = gradient (rise Ã· run)","b = y-intercept (where line crosses y-axis)","Positive gradient: goes up left to right","Find x-intercept: set y = 0 and solve"],formula:"y = mx + b"},
    {id:"simultaneous",emoji:"âï¸",title:"Simultaneous Equations",summary:"Two equations, two unknowns â find values that satisfy both.",keyFacts:["Substitution: sub one equation into the other","Elimination: add/subtract to remove a variable","Solution is the intersection point of two lines","Check by substituting into BOTH original equations","Parallel lines â no solution, same line â infinite solutions"]},
    {id:"quadratics",emoji:"âª",title:"Quadratic Equations",summary:"Equations with xÂ² as highest power. Graphs are parabolas.",keyFacts:["Standard form: axÂ² + bx + c = 0","Factorise: (x+p)(x+q) = 0 â x = -p or x = -q","Quadratic formula: x = (-b Â± â(bÂ²-4ac)) / 2a","Discriminant bÂ²-4ac: >0 two solutions, =0 one, <0 none","Vertex is the turning point of the parabola"],formula:"x = (-b Â± â(bÂ²-4ac)) / 2a"},
  ],
  "Functions & Graphs": [
    {id:"function-notation",emoji:"f(x)",title:"Functions & Function Notation",summary:"A function maps each input to exactly one output.",keyFacts:["f(x) means 'f of x' â substitute x into the rule","Domain: all valid input values","Range: all possible output values","Vertical line test: each x has only one y","Composite: f(g(x)) â apply g first, then f"]},
    {id:"transformations",emoji:"â",title:"Transformations of Graphs",summary:"How graphs shift, stretch and flip when we change the function.",keyFacts:["y = f(x) + k: shifts UP k units","y = f(x) - k: shifts DOWN k units","y = f(x - h): shifts RIGHT h units","y = f(x + h): shifts LEFT h units","y = af(x): vertical stretch by factor a","y = -f(x): reflection in x-axis"]},
    {id:"exp-log",emoji:"eË£",title:"Exponential & Logarithmic Functions",summary:"Exponentials model growth/decay. Logs are their inverses.",keyFacts:["y = aË£: exponential growth if a > 1, decay if 0 < a < 1","y = eË£: natural exponential (e â 2.718)","ln(x) = log_e(x): natural logarithm","log(ab) = log a + log b","log(a/b) = log a - log b"],formula:"y = eË£  â  x = ln(y)"},
  ],
  "Calculus â Differentiation": [
    {id:"power-rule",emoji:"d/dx",title:"Differentiation Rules",summary:"Shortcuts for finding derivatives of common functions.",keyFacts:["Power rule: d/dx(xâ¿) = nxâ¿â»Â¹","d/dx(eË£) = eË£","d/dx(ln x) = 1/x","d/dx(sin x) = cos x","d/dx(cos x) = -sin x"],formula:"d/dx(xâ¿) = nxâ¿â»Â¹"},
    {id:"chain-rule",emoji:"â",title:"Chain, Product & Quotient Rules",summary:"Rules for differentiating composite and combined functions.",keyFacts:["Chain rule: d/dx[f(g(x))] = f'(g(x)) Ã g'(x)","Product rule: d/dx[uv] = u'v + uv'","Quotient rule: d/dx[u/v] = (u'v - uv') / vÂ²","Chain rule tip: 'outside Ã derivative of inside'","Most common exam rule: chain rule"],formula:"Chain: dy/dx = dy/du Ã du/dx"},
    {id:"applications-diff",emoji:"ð",title:"Applications of Differentiation",summary:"Using derivatives to analyse function behaviour and solve optimisation.",keyFacts:["f'(x) = 0 at stationary points (local max/min)","f'(x) > 0: function increasing","f'(x) < 0: function decreasing","f''(x) > 0: minimum; f''(x) < 0: maximum","Tangent gradient at x=a is f'(a)"],},
  ],
  "Calculus â Integration": [
    {id:"antidiff",emoji:"â«",title:"Antidifferentiation & Indefinite Integrals",summary:"The reverse of differentiation â finding the original function.",keyFacts:["â«xâ¿ dx = xâ¿âºÂ¹/(n+1) + C  (n â  -1)","â«eË£ dx = eË£ + C","â«(1/x) dx = ln|x| + C","â«sin x dx = -cos x + C","Always add + C (constant of integration)"],formula:"â«xâ¿ dx = xâ¿âºÂ¹/(n+1) + C"},
    {id:"definite-integrals",emoji:"â«âáµ",title:"Definite Integrals & Area",summary:"Calculates the exact area between a curve and the x-axis.",keyFacts:["â«âáµ f(x)dx = F(b) - F(a)  where F' = f","Area above x-axis is positive","Area below x-axis is negative â use absolute value for area","Area between two curves: â«(top - bottom)dx","Fundamental theorem connects differentiation and integration"],formula:"â«âáµ f(x)dx = F(b) - F(a)"},
  ],
  "Organic Chemistry": [
    {id:"functional-groups",emoji:"âï¸",title:"Functional Groups",summary:"Organic compounds are classified by their functional groups which determine their reactions.",keyFacts:["Alkanes: C-C single bonds, suffix -ane (methane, ethane, propane)","Alkenes: C=C double bond, suffix -ene","Alcohols: -OH group, suffix -ol (ethanol)","Carboxylic acids: -COOH, suffix -oic acid","Esters: -COO-, formed from acid + alcohol (condensation)"]},
    {id:"reaction-types",emoji:"ð",title:"Organic Reaction Types",summary:"The key reactions organic compounds undergo.",keyFacts:["Addition: adds across C=C double bond (alkenes + Hâ, Brâ, HâO)","Substitution: replaces H with another atom (alkanes + Clâ)","Condensation: two molecules join, releasing HâO","Hydrolysis: breaks bonds using water (reverse of condensation)","Combustion: burns in Oâ â COâ + HâO + energy"]},
  ],
  "Electrochemistry": [
    {id:"redox",emoji:"â¡",title:"Oxidation & Reduction (OIL RIG)",summary:"Redox reactions involve transfer of electrons between species.",keyFacts:["OIL: Oxidation Is Loss of electrons","RIG: Reduction Is Gain of electrons","Oxidising agent: accepts electrons (gets reduced itself)","Reducing agent: donates electrons (gets oxidised itself)","Assign oxidation numbers to track electron transfer"],formula:"OIL RIG â Oxidation Is Loss, Reduction Is Gain"},
    {id:"galvanic-cells",emoji:"ð",title:"Galvanic Cells",summary:"Convert chemical energy to electrical energy via spontaneous redox.",keyFacts:["Anode: oxidation occurs (negative terminal)","Cathode: reduction occurs (positive terminal)","Salt bridge: maintains electrical neutrality","EMF = EÂ°(cathode) - EÂ°(anode)","Electrons flow: anode â external circuit â cathode"],formula:"EMF = EÂ°cathode - EÂ°anode"},
    {id:"electrolysis",emoji:"ð",title:"Electrolytic Cells",summary:"Use electrical energy to drive non-spontaneous chemical reactions.",keyFacts:["Anode: oxidation (connected to + terminal)","Cathode: reduction (connected to - terminal)","Faraday's law: mass = (I Ã t Ã M) / (n Ã F)","Applications: electroplating, aluminium smelting","More current Ã more time = more product"],formula:"m = ItM / nF"},
  ],
  "Energetics & Thermodynamics": [
    {id:"enthalpy",emoji:"ÎH",title:"Enthalpy Changes",summary:"ÎH measures heat released or absorbed in a chemical reaction.",keyFacts:["Exothermic: ÎH < 0 (heat released, products lower energy)","Endothermic: ÎH > 0 (heat absorbed, products higher energy)","q = mcÎT for calorimetry calculations","Standard conditions: 25Â°C, 1 atm, 1 mol/L","Bond breaking absorbs energy; bond forming releases energy"],formula:"q = mcÎT"},
    {id:"gibbs",emoji:"ÎG",title:"Gibbs Free Energy & Spontaneity",summary:"ÎG predicts whether a reaction occurs spontaneously.",keyFacts:["ÎG = ÎH - TÎS","ÎG < 0: spontaneous (will occur without energy input)","ÎG > 0: non-spontaneous","ÎG = 0: at equilibrium","ÎS: entropy â measure of disorder (gases > liquids > solids)"],formula:"ÎG = ÎH - TÎS"},
  ],
  "Equilibrium & Acids/Bases": [
    {id:"equilibrium",emoji:"â",title:"Dynamic Equilibrium & Le Chatelier's",summary:"A reversible reaction where forward and reverse rates are equal.",keyFacts:["Equilibrium: concentrations constant (not necessarily equal)","Le Chatelier: system opposes changes to restore equilibrium","Add reactant â shifts toward products","Increase temperature â shifts toward endothermic direction","Increase pressure â shifts toward fewer moles of gas","Catalyst: speeds up reaching equilibrium, doesn't shift position"]},
    {id:"ph",emoji:"pH",title:"pH, Acids & Bases",summary:"pH measures hydrogen ion concentration on a logarithmic scale.",keyFacts:["pH = -log[Hâº]","pH < 7: acidic; pH 7: neutral; pH > 7: basic (alkaline)","Strong acids fully dissociate: HCl, HâSOâ, HNOâ","Weak acids partially dissociate: CHâCOOH (acetic acid)","pH + pOH = 14 at 25Â°C","Kw = [Hâº][OHâ»] = 1Ã10â»Â¹â´"],formula:"pH = -log[Hâº]   pOH = -log[OHâ»]   pH + pOH = 14"},
  ],
  "Motion & Forces": [
    {id:"kinematics",emoji:"ð",title:"Kinematics â Equations of Motion",summary:"Describes motion using displacement, velocity, acceleration and time.",keyFacts:["v = u + at","s = ut + Â½atÂ²","vÂ² = uÂ² + 2as","s = Â½(u+v)t","Only valid for CONSTANT acceleration","v=final, u=initial, a=acceleration, s=displacement, t=time"],formula:"v = u + at  |  s = ut + Â½atÂ²  |  vÂ² = uÂ² + 2as"},
    {id:"newtons-laws",emoji:"âï¸",title:"Newton's Three Laws",summary:"Fundamental laws describing how forces affect the motion of objects.",keyFacts:["1st Law (Inertia): object stays at rest or moves at constant velocity unless a net force acts","2nd Law: F = ma (net force = mass Ã acceleration)","3rd Law: every action has equal and opposite reaction pair","Weight W = mg where g = 9.8 m/sÂ²","Net force = vector sum of ALL forces acting"],formula:"F = ma  |  W = mg"},
    {id:"momentum",emoji:"ð¥",title:"Momentum & Impulse",summary:"Momentum is mass Ã velocity. Impulse is change in momentum.",keyFacts:["p = mv (momentum, kgâm/s)","Impulse J = FÎt = Îp","Conservation: total momentum unchanged if no external force","Elastic collision: KE and momentum both conserved","Inelastic: only momentum conserved (objects may stick together)"],formula:"p = mv  |  J = FÎt = Îp"},
    {id:"energy-work",emoji:"â¡",title:"Work, Energy & Power",summary:"Work is done when a force moves an object. Energy is the capacity to do work.",keyFacts:["W = Fs cos Î¸","KE = Â½mvÂ²","GPE = mgh","Conservation: total mechanical energy is constant (no friction)","P = W/t = Fv","Efficiency = useful output / total input Ã 100%"],formula:"W = Fs  |  KE = Â½mvÂ²  |  PE = mgh  |  P = W/t"},
    {id:"circular",emoji:"â­",title:"Circular Motion",summary:"Objects moving in circles experience centripetal acceleration toward the centre.",keyFacts:["Centripetal acceleration: a = vÂ²/r","Centripetal force: F = mvÂ²/r (directed toward centre, NOT outward)","Period T: time for one complete revolution","Frequency f = 1/T","v = 2Ïr/T","Examples: satellites, cars on bends, spinning on a string"],formula:"F = mvÂ²/r  |  a = vÂ²/r"},
  ],
  "Electricity & Magnetism": [
    {id:"dc-circuits",emoji:"ð",title:"DC Circuits & Ohm's Law",summary:"Circuits with direct current. All obey Ohm's Law: V = IR.",keyFacts:["Ohm's Law: V = IR (Voltage = Current Ã Resistance)","Series: same current everywhere, voltages add, R_total = Râ+Râ+...","Parallel: same voltage, currents add, 1/R = 1/Râ+1/Râ+...","Power: P = VI = IÂ²R = VÂ²/R","Kirchhoff's voltage law: sum of EMFs = sum of voltage drops"],formula:"V = IR  |  P = VI  |  Series: R=Râ+Râ"},
    {id:"em-induction",emoji:"ð§²",title:"Electromagnetic Induction",summary:"A changing magnetic field induces an EMF and current in a conductor.",keyFacts:["Faraday's law: EMF = -ÎÎ¦/Ît","Lenz's law: induced current OPPOSES the change that caused it","Magnetic flux: Î¦ = BA cos Î¸","Generator converts mechanical â electrical energy","Transformer: Vâ/Vâ = Nâ/Nâ (turns ratio)"],formula:"EMF = -ÎÎ¦/Ît  |  Vâ/Vâ = Nâ/Nâ"},
  ],
  "Cell Biology & Regulation": [
    {id:"cell-structure",emoji:"ð¬",title:"Cell Structure & Organelles",summary:"All living things are made of cells with specific organelles performing specific functions.",keyFacts:["Nucleus: contains DNA, controls cell activities","Mitochondria: aerobic respiration â ATP (energy)","Chloroplasts: photosynthesis (plants/algae only)","Ribosomes: protein synthesis (translation)","Cell membrane: controls entry/exit (phospholipid bilayer)","Endoplasmic reticulum: transport network (rough = proteins, smooth = lipids)"]},
    {id:"dna-protein",emoji:"ð§¬",title:"DNA, Transcription & Translation",summary:"DNA stores genetic information and directs protein synthesis.",keyFacts:["DNA: double helix, nucleotides, base pairs A-T and G-C","Transcription: DNA â mRNA (in nucleus)","Translation: mRNA â protein (at ribosomes)","Codon: 3 bases on mRNA codes for one amino acid","Start codon: AUG; Stop codons: UAA, UAG, UGA","Gene expression is controlled â not all genes are always active"]},
    {id:"respiration",emoji:"â¡",title:"Cellular Respiration",summary:"Releases energy from glucose to produce ATP.",keyFacts:["Overall: CâHââOâ + 6Oâ â 6COâ + 6HâO + ATP","Glycolysis: glucose â pyruvate, in cytoplasm, 2 ATP, no Oâ needed","Krebs cycle: pyruvate â COâ, in mitochondrial matrix, 2 ATP","ETC: uses Oâ, produces ~32 ATP, in inner mitochondrial membrane","Anaerobic (no Oâ): fermentation â lactic acid (animals) or ethanol (yeast)"],formula:"CâHââOâ + 6Oâ â 6COâ + 6HâO + ~36 ATP"},
  ],
  "Nervous System & Brain": [
    {id:"neurons",emoji:"ð§ ",title:"Neurons & Synaptic Transmission",summary:"Neurons are specialised cells that transmit electrical and chemical signals.",keyFacts:["Sensory neurons: carry signals from sense organs to brain","Motor neurons: carry signals from brain to muscles/glands","Interneurons: connect sensory and motor pathways in the CNS","Action potential: electrical signal along axon (all-or-nothing response)","Synapse: gap between neurons â neurotransmitters diffuse across","Key neurotransmitters: serotonin (mood), dopamine (reward), GABA (calming)"]},
  ],
  "Learning & Memory": [
    {id:"classical-conditioning",emoji:"ð",title:"Classical Conditioning (Pavlov)",summary:"Learning by association â a neutral stimulus becomes linked to an automatic response.",keyFacts:["US: Unconditioned Stimulus (food naturally causes salivation)","UR: Unconditioned Response (automatic response to US)","NS: Neutral Stimulus (bell â no response initially)","CS: Conditioned Stimulus (bell after pairing with food)","CR: Conditioned Response (salivation to bell alone)","Extinction: CR weakens if CS presented repeatedly without US"]},
    {id:"operant-conditioning",emoji:"ð",title:"Operant Conditioning (Skinner)",summary:"Learning through consequences â behaviour strengthened or weakened by outcomes.",keyFacts:["Positive reinforcement: add pleasant stimulus â behaviour increases","Negative reinforcement: remove unpleasant â behaviour increases (e.g. taking painkiller removes pain)","Positive punishment: add unpleasant â behaviour decreases","Negative punishment: remove pleasant â behaviour decreases","Variable ratio schedule: most resistant to extinction (e.g. pokies, social media)"]},
    {id:"memory-models",emoji:"ð§ ",title:"Atkinson-Shiffrin Memory Model",summary:"Information flows through sensory, short-term and long-term memory stores.",keyFacts:["Sensory memory: fraction of a second (iconic = visual; echoic = auditory)","Short-term memory (STM): 20-30 seconds, capacity of 7Â±2 chunks","Long-term memory (LTM): potentially unlimited capacity and duration","Maintenance rehearsal transfers STM â LTM","Levels of processing: deeper processing = better encoding and recall"]},
  ],
  "The Victorian Court System": [
    {id:"court-hierarchy",emoji:"âï¸",title:"Court Hierarchy",summary:"Courts are arranged by jurisdiction and status â higher courts set binding precedent.",keyFacts:["Magistrates Court: summary offences, civil up to $100K","County Court: serious indictable offences, civil up to $1M","Supreme Court: most serious crimes, unlimited civil jurisdiction","Court of Appeal: hears appeals from Magistrates and County Courts","High Court of Australia: final court of appeal, constitutional matters","Higher courts bind lower courts through doctrine of precedent"]},
  ],
  "Criminal Law": [
    {id:"elements-crime",emoji:"ð",title:"Elements of a Crime",summary:"Every crime requires two elements proved beyond reasonable doubt.",keyFacts:["Actus reus: the guilty act (physical element)","Mens rea: the guilty mind (mental/intention element)","Both must be present AND occur at the same time","Strict liability offences: actus reus only (e.g. speeding)","Standard of proof in criminal law: beyond reasonable doubt","Burden of proof: prosecution must prove guilt (accused is presumed innocent)"]},
  ],
  "Functions & Graphs (Year 9-10)": [
    {id:"linear-graphs",emoji:"ð",title:"Linear & Non-Linear Graphs",summary:"Graphs of equations showing relationships between variables.",keyFacts:["Linear: y = mx + b â straight line","Gradient m = rise/run","Parabola: y = axÂ² + bx + c â U-shaped curve","Exponential: y = aË£ â rapid growth or decay","Transformations shift, stretch and flip graphs"]},
    {id:"parabolas",emoji:"âª",title:"Parabolas & Quadratic Graphs",summary:"U-shaped graphs of quadratic functions y = axÂ² + bx + c.",keyFacts:["Vertex: turning point of the parabola","Axis of symmetry: x = -b/2a","x-intercepts: where parabola crosses x-axis (roots)","a > 0: opens upward (happy face) âª","a < 0: opens downward (sad face) â©"],formula:"y = axÂ² + bx + c"},
  ],
  "Biology (Year 9-10)": [
    {id:"cells",emoji:"ð¬",title:"Cells & Cell Division",summary:"All living things are made of cells. New cells form through division.",keyFacts:["Plant cells: cell wall, chloroplasts, large vacuole (not in animal cells)","Animal cells: no cell wall, no chloroplasts, small vacuoles","Mitosis: produces 2 identical daughter cells (growth and repair)","Meiosis: produces 4 gametes with half the chromosomes (reproduction)","DNA in nucleus controls all cell activities"]},
    {id:"genetics",emoji:"ð§¬",title:"Genetics & Inheritance",summary:"Traits are passed from parents to offspring through genes.",keyFacts:["Genes are sections of DNA on chromosomes","Alleles: different versions of the same gene","Dominant allele (B) masks recessive (b) â BB or Bb shows dominant trait","Recessive only shows if both alleles are recessive (bb)","Punnett squares predict probability of offspring traits"]},
    {id:"evolution",emoji:"ð¦",title:"Evolution & Natural Selection",summary:"Species change over time through natural selection acting on variation.",keyFacts:["Variation: individuals differ within a species","Survival of the fittest: better-adapted individuals survive and reproduce","Inherited traits: successful adaptations passed to offspring","Evidence: fossil record, comparative anatomy, DNA similarities","Speciation: isolated populations diverge into separate species"]},
  ],
  "Chemistry (Year 9-10)": [
    {id:"atomic-structure",emoji:"âï¸",title:"Atomic Structure",summary:"Atoms have protons, neutrons and electrons arranged in a specific way.",keyFacts:["Protons: positive charge, in nucleus","Neutrons: no charge, in nucleus","Electrons: negative charge, in shells around nucleus","Atomic number = number of protons","Mass number = protons + neutrons","Ions: atoms that have gained or lost electrons"],formula:"Atomic number = protons | Mass number = protons + neutrons"},
    {id:"periodic-table",emoji:"ðï¸",title:"The Periodic Table",summary:"Elements arranged by atomic number in groups (columns) and periods (rows).",keyFacts:["Group: column â elements have similar properties","Period: row â same number of electron shells","Metals: left side, conductors, lose electrons to form positive ions","Non-metals: right side, insulators, gain electrons to form negative ions","Noble gases: Group 18, full outer shell, very unreactive","Valence electrons determine reactivity and bonding"]},
    {id:"chemical-reactions",emoji:"âï¸",title:"Chemical Reactions",summary:"Substances rearrange their atoms to form new products with different properties.",keyFacts:["Reactants â Products (written as chemical equation)","Law of conservation of mass: total mass doesn't change","Balancing equations: same number of each atom on both sides","Exothermic: releases heat (combustion, neutralisation)","Endothermic: absorbs heat (photosynthesis, cooking)","Indicators: show if a substance is acid, neutral or base"],},
  ],
  "Physics (Year 9-10)": [
    {id:"motion-forces",emoji:"ð",title:"Motion & Forces",summary:"Motion is described by distance, speed, velocity and acceleration. Forces cause changes in motion.",keyFacts:["Speed = distance Ã· time","Velocity: speed with direction (vector)","Acceleration: change in velocity Ã· time","Newton's 1st Law: object stays at rest or constant velocity unless a net force acts","Newton's 2nd Law: F = ma","Newton's 3rd Law: every action has equal and opposite reaction"],formula:"speed = distance/time | F = ma"},
    {id:"energy-waves",emoji:"ð",title:"Energy & Waves",summary:"Energy comes in many forms and can be transferred and transformed.",keyFacts:["KE = Â½mvÂ² (kinetic energy)","GPE = mgh (gravitational potential energy)","Conservation of energy: total energy stays constant","Waves transfer energy without transferring matter","Frequency (Hz) Ã wavelength (m) = wave speed (m/s)","Light: reflection, refraction, dispersion"],formula:"wave speed = frequency Ã wavelength"},
    {id:"electricity",emoji:"â¡",title:"Electricity & Circuits",summary:"Electric current is the flow of electrons through a conductor.",keyFacts:["Ohm's Law: V = IR (Voltage = Current Ã Resistance)","Series circuit: same current everywhere, voltages add","Parallel circuit: same voltage, currents add","Power = Voltage Ã Current (P = VI)","Insulators: don't conduct electricity (rubber, plastic)","Conductors: allow current flow (copper, aluminium)"],formula:"V = IR | P = VI"},
  ],
  "Aggregate Demand & Supply": [
    {id:"aggregate-demand",emoji:"ð",title:"Aggregate Demand (AD)",summary:"Total demand for all goods and services in an economy at a given price level.",keyFacts:["AD = C + I + G + (X - M)","C = consumption (households), I = investment (businesses)","G = government spending, X = exports, M = imports","AD shifts right: more spending, lower interest rates, higher confidence","Multiplier effect: initial spending increase amplified through economy"],formula:"AD = C + I + G + (X-M)"},
    {id:"aggregate-supply",emoji:"ð­",title:"Aggregate Supply (AS)",summary:"Total production of goods and services at various price levels.",keyFacts:["Short-run AS: upward sloping (higher prices â more production)","Long-run AS: vertical at potential GDP (full employment output)","AS shifts right: lower input costs, improved technology, more workers","Supply shock: sudden disruption to AS (e.g. oil price spike)","Equilibrium: AD = AS determines price level and real output"]},
  ],

  // ââ VCE ENGLISH ââ
  "Reading & Responding to Texts": [
    {id:"analysing-authors",emoji:"ð",title:"Analysing How Authors Construct Meaning",summary:"Authors deliberately choose language, structure and literary techniques to shape meaning and influence readers.",keyFacts:["Language features: diction (word choice), syntax, tone, imagery, figurative language","Structure: how a text is organised â chronological, non-linear, cyclical","Perspective: the point of view from which the story is told","Context: historical, social and cultural circumstances of the text","Metalanguage: specific vocabulary for discussing texts (e.g. symbolism, characterisation, motif)"]},
    {id:"themes-values",emoji:"ð¡",title:"Ideas, Themes & Values",summary:"Identifying and discussing the central ideas and values explored in a text.",keyFacts:["Theme: a central idea explored throughout the text","Values: beliefs and attitudes promoted or critiqued by the text","Avoid plot summary â focus on what the text is saying about human experience","Use evidence (quotes) to support your interpretation","Consider how different readers might interpret the same text differently"]},
    {id:"written-response",emoji:"âï¸",title:"Writing a Text Response Essay",summary:"A structured argument about a text using evidence and sophisticated analysis.",keyFacts:["Introduction: clear contention (your argument about the text)","Body paragraphs: TEEL â Topic sentence, Evidence, Explanation, Link","Embed quotes: integrate short quotes into your own sentences","Analysis: explain HOW the technique creates meaning, not just WHAT it does","Conclusion: synthesise your argument â don't just repeat introduction","Avoid 'the author wants us to think...' â use 'the author suggests/explores/reveals'"]},
  ],
  "Creating & Presenting": [
    {id:"writing-for-purpose",emoji:"âï¸",title:"Writing for a Specific Audience & Purpose",summary:"Adapting your writing style, form and language to suit the task, audience and context.",keyFacts:["Purpose: to inform, persuade, entertain, reflect, or a combination","Audience: who will read your text â formal/informal, specific group","Form: essay, speech, short story, letter, blog post, interview","Voice: the distinctive style and personality of the writer","Register: level of formality â match to audience and purpose"]},
    {id:"persuasive-techniques",emoji:"ð£ï¸",title:"Persuasive Writing Techniques",summary:"Rhetorical devices and language strategies used to persuade an audience.",keyFacts:["Ethos: appeal to credibility/authority ('As an expert...')","Pathos: appeal to emotion â evoke fear, sympathy, anger, hope","Logos: appeal to logic â statistics, evidence, reasoned argument","Anecdote: personal story to make argument relatable","Rhetorical question: engages reader, implies obvious answer","Rule of three: three examples or points for emphasis and rhythm"]},
    {id:"imaginative-writing",emoji:"ð¨",title:"Imaginative Writing",summary:"Creating original creative texts using narrative techniques and craft.",keyFacts:["Show don't tell: use specific sensory details rather than stating emotions","Characterisation: reveal character through dialogue, action, thought, description","Setting: establishes atmosphere, mood and context","Conflict: drives narrative â internal (character vs self) or external","Voice: first person (I) or third person (he/she/they)","Structure: consider beginning, complication, climax, resolution"]},
  ],
  "Analysing Argument": [
    {id:"identifying-argument",emoji:"ð",title:"Identifying Contention & Arguments",summary:"Analysing how a writer constructs a persuasive argument.",keyFacts:["Contention: the main argument or position of the author","Supporting arguments: specific points used to support the contention","Identify: Who? What audience? What position? What form? What context?","Arguments can be explicit (directly stated) or implicit (implied)","Consider: what is NOT said as well as what is said"]},
    {id:"analysing-language",emoji:"ð¬",title:"Analysing Persuasive Language",summary:"Examining how specific language choices are used to persuade.",keyFacts:["Emotive language: words chosen to trigger emotional response","Loaded language: words with strong positive or negative connotations","Inclusive language: 'we', 'our' â positions reader as part of the argument","Generalisation: broad statements presented as universal truth","Visuals: images, cartoons, graphs used to support or enhance argument","Tone: the attitude conveyed by the writer (outraged, concerned, sarcastic)"]},
    {id:"formal-analysis",emoji:"ð",title:"Writing a Formal Analytical Response",summary:"A structured analysis of how argument and language are used to persuade.",keyFacts:["Do NOT share your own opinion â analyse the author's techniques","Structure: Introduction â Body paragraphs by argument/technique â Conclusion","Begin body paragraphs with what the author argues, then HOW they persuade","Metalanguage: name the technique, quote it, explain its effect","Discuss the intended effect on the audience","Present tense: 'The author argues...' not 'The author argued...'"]},
  ],

  // ââ VCE MATHS ââ
  "Probability & Statistics (Methods)": [
    {id:"discrete-rv",emoji:"ð²",title:"Discrete Random Variables",summary:"Variables that can only take countable values, each with an associated probability.",keyFacts:["P(X=x) â¥ 0 for all x, and sum of all probabilities = 1","E(X) = Î£ xÂ·P(X=x) â the expected (mean) value","Var(X) = E(XÂ²) â [E(X)]Â² â how spread out values are","SD(X) = âVar(X)","Binomial distribution: X ~ B(n,p) â n trials, probability p each"],formula:"E(X) = Î£ xÂ·P(X=x) | Var(X) = E(XÂ²) - [E(X)]Â²"},
    {id:"normal-dist-methods",emoji:"ð",title:"Normal Distribution",summary:"Bell-shaped distribution described by mean Î¼ and standard deviation Ï.",keyFacts:["68% of data within 1Ï of mean","95% within 2Ï, 99.7% within 3Ï (68-95-99.7 rule)","Standardise: z = (x-Î¼)/Ï","Use CAS to find probabilities: normalcdf(lower, upper, Î¼, Ï)","Inverse normal: find x given a probability"],formula:"z = (x - Î¼) / Ï"},
    {id:"confidence-intervals",emoji:"ð",title:"Statistical Inference & Confidence Intervals",summary:"Using sample data to make inferences about population parameters.",keyFacts:["Sample proportion pÌ estimates population proportion p","Standard error: SE = â(pÌ(1-pÌ)/n)","95% confidence interval: pÌ Â± 1.96 Ã SE","Larger sample â narrower interval â more precise estimate","Margin of error = 1.96 Ã SE for 95% CI"],formula:"95% CI: pÌ Â± 1.96â(pÌ(1-pÌ)/n)"},
  ],

  // ââ VCE SPECIALIST MATHS ââ
  "Algebra & Number Systems": [
    {id:"complex-numbers",emoji:"i",title:"Complex Numbers",summary:"Numbers of the form a + bi where i = â(-1).",keyFacts:["i = â(-1), iÂ² = -1, iÂ³ = -i, iâ´ = 1","Rectangular form: z = a + bi (a = real part, b = imaginary part)","Modulus: |z| = â(aÂ² + bÂ²)","Argument: Î¸ = arctan(b/a) â angle from positive real axis","Polar form: z = r(cos Î¸ + i sin Î¸) = rÂ·cis(Î¸)","Complex conjugate: zÌ = a - bi"],formula:"z = a + bi | |z| = â(aÂ² + bÂ²)"},
    {id:"proof",emoji:"â´",title:"Mathematical Proof",summary:"Rigorous logical arguments that establish mathematical statements as definitely true.",keyFacts:["Direct proof: assume premises, deduce conclusion step by step","Proof by contradiction: assume opposite is true, derive contradiction","Mathematical induction: prove base case, then prove if P(k) true â P(k+1) true","Counterexample: one example that disproves a universal statement","Always state what you're proving and every step must follow logically"]},
  ],

  // ââ VCE GENERAL MATHS ââ
  "Data Analysis": [
    {id:"univariate-data",emoji:"ð",title:"Univariate Data Analysis",summary:"Analysing data with a single variable using numerical and graphical methods.",keyFacts:["Centre: mean (affected by outliers), median (resistant to outliers)","Spread: range, IQR, standard deviation","Five-number summary: min, Q1, median, Q3, max","Box plots: display five-number summary, show outliers","Outlier: more than 1.5 Ã IQR above Q3 or below Q1","Symmetry vs skewness: affects which measure of centre to use"]},
    {id:"bivariate-data",emoji:"ð",title:"Bivariate Data & Regression",summary:"Analysing the relationship between two numerical variables.",keyFacts:["Scatterplot: displays relationship between two variables","Pearson's r: correlation coefficient (-1 to 1)","r close to Â±1: strong linear relationship; r near 0: no linear relationship","Least squares regression line: y = a + bx (line of best fit)","Interpolation: predicting within data range (reliable)","Extrapolation: predicting outside data range (unreliable)"],formula:"r measures strength and direction of linear relationship"},
    {id:"time-series",emoji:"ð",title:"Time Series Analysis",summary:"Data collected over time â identifying trends, seasonality and cycles.",keyFacts:["Trend: long-term increase or decrease","Seasonal variation: regular pattern within each year","Smoothing: moving averages reduce random variation to reveal trend","Seasonal indices: show how much each season differs from average","Deseasonalise: remove seasonal variation to see underlying trend"]},
  ],
  "Financial Arithmetic": [
    {id:"simple-compound",emoji:"ð°",title:"Simple & Compound Interest",summary:"Two methods of calculating interest on investments and loans.",keyFacts:["Simple interest: I = Prt (Principal Ã rate Ã time)","Compound interest: A = P(1 + r)â¿","Compounding period affects final amount (more frequent = more interest)","Effective annual rate: accounts for compounding frequency","Rule of 72: years to double â 72 / interest rate %"],formula:"Simple: I = Prt | Compound: A = P(1+r)â¿"},
    {id:"loans-annuities",emoji:"ð ",title:"Loans, Annuities & Reducible Interest",summary:"Calculating repayments and future values for financial products.",keyFacts:["Reducible interest: calculated on outstanding balance (most loans)","Annuity: regular equal payments over a fixed period","Present value: current value of future payments","Future value: total accumulated after regular deposits","Recurrence relation: Vâââ = Vâ(1+r) - d"],formula:"Vâââ = Vâ(1+r) - d"},
  ],

  // ââ VCE BIOLOGY ââ
  "Genetics & Inheritance": [
    {id:"mendelian",emoji:"ð§¬",title:"Mendelian Genetics",summary:"Gregor Mendel's laws describing how traits are inherited from parents.",keyFacts:["Dominant allele (B): expressed even with one copy","Recessive allele (b): only expressed when two copies present (bb)","Homozygous: same alleles (BB or bb); Heterozygous: different alleles (Bb)","Punnett square: predicts offspring ratios","Monohybrid cross: one trait; Dihybrid cross: two traits","Law of segregation: alleles separate during gamete formation"]},
    ...
    {id:"biotechnology",emoji:"ð¬",title:"Biotechnology & Gene Technology",summary:"Techniques used to manipulate, analyse and apply genetic material.",keyFacts:["PCR: Polymerase Chain Reaction â amplifies tiny DNA samples","Gel electrophoresis: separates DNA fragments by size","DNA profiling: unique pattern used in forensics and paternity testing","CRISPR-Cas9: precise gene editing â cut and replace DNA sequences","Recombinant DNA: inserting gene from one organism into another","GMOs: genetically modified organisms â ethical considerations apply"]},
  ],
  "Evolution & Biodiversity": [
    {id:"natural-selection",emoji:"ð¦",title:"Natural Selection & Evolution",summary:"Populations change over time as better-adapted individuals survive and reproduce.",keyFacts:["Variation: individuals differ within a population","Heritability: advantageous traits are passed to offspring","Differential reproduction: better-adapted individuals have more offspring","Over many generations: allele frequencies shift","Evidence: fossil record, comparative anatomy, DNA, biogeography","Antibiotic resistance: natural selection in action â real-world example"]},
    {id:"speciation",emoji:"ð¿",title:"Speciation & Classification",summary:"How new species form and how organisms are classified into groups.",keyFacts:["Species: organisms that can interbreed to produce fertile offspring","Allopatric speciation: geographic isolation leads to divergence","Sympatric speciation: new species form in same area (e.g. polyploidy in plants)","Phylogenetic trees (cladograms): show evolutionary relationships","Binomial nomenclature: Genus species (e.g. Homo sapiens)","Domains: Bacteria, Archaea, Eukarya â three domains of life"]},
  ],
  "Immunity & Disease": [
    {id:"innate-immunity",emoji:"ð¡ï¸",title:"Innate (Non-Specific) Immunity",summary:"The body's first line of defence â rapid, non-specific response to pathogens.",keyFacts:["Physical barriers: skin, mucus membranes, cilia, stomach acid","Inflammation: redness, heat, swelling â brings immune cells to site","Phagocytes: neutrophils and macrophages engulf and destroy pathogens","Fever: elevated temperature slows pathogen reproduction","Complement system: proteins that attack pathogens and signal immune cells"]},
    {id:"adaptive-immunity",emoji:"ð¯",title:"Adaptive (Specific) Immunity",summary:"Targeted immune response that develops over days and creates memory.",keyFacts:["Antigens: molecules on pathogen surface that trigger immune response","B lymphocytes: produce antibodies specific to the antigen","T helper cells: coordinate immune response","T cytotoxic cells: kill infected body cells directly","Clonal selection: B and T cells multiply rapidly when activated","Memory cells: remain after infection â faster response if exposed again","Antibodies: Y-shaped proteins that bind to specific antigens"]},
    {id:"vaccination",emoji:"ð",title:"Vaccination & Herd Immunity",summary:"Using vaccines to stimulate adaptive immunity without causing disease.",keyFacts:["Vaccine: contains weakened/dead pathogen or antigen fragments","Stimulates production of antibodies and memory cells","Herd immunity: enough people immune â pathogen can't spread","Herd immunity threshold varies by disease (measles ~95%, COVID ~70%)","Types: live attenuated, killed, subunit, mRNA vaccines","Booster shots: refresh memory cell populations over time"]},
  ],

  // ââ VCE PHYSICS ââ
  "Waves & Light": [
    {id:"wave-properties",emoji:"ð",title:"Wave Properties",summary:"All waves share fundamental properties described by frequency, wavelength, amplitude and speed.",keyFacts:["Wave speed v = fÎ» (frequency Ã wavelength)","Frequency f: cycles per second (Hz)","Wavelength Î»: distance between successive crests","Amplitude: maximum displacement from rest position","Transverse waves: displacement perpendicular to direction (light, water)","Longitudinal waves: displacement parallel to direction (sound)"],formula:"v = fÎ»  |  T = 1/f"},
    {id:"interference",emoji:"ã°ï¸",title:"Interference & Diffraction",summary:"Waves combine (interference) and spread around obstacles (diffraction).",keyFacts:["Constructive interference: waves add together (crest + crest)","Destructive interference: waves cancel (crest + trough)","Path difference: difference in distance each wave travels","Constructive: path difference = nÎ» (whole number of wavelengths)","Destructive: path difference = (n + Â½)Î»","Diffraction: maximum when gap width â wavelength"],formula:"Constructive: Îd = nÎ» | Destructive: Îd = (n+Â½)Î»"},
    {id:"photoelectric",emoji:"â¡",title:"Photoelectric Effect & Quantum Model",summary:"Light behaves as particles (photons) â evidence for quantum nature of light.",keyFacts:["Photon: packet of light energy, E = hf","Threshold frequency: minimum frequency to eject electrons","Increasing intensity: more electrons ejected (not faster ones)","Increasing frequency: electrons ejected with more kinetic energy","Einstein's equation: KE_max = hf - W (W = work function)","Wave-particle duality: light (and matter) exhibits both wave and particle behaviour"],formula:"E = hf  |  KE_max = hf - W"},
  ],
  "Modern Physics": [
    {id:"special-relativity",emoji:"â¡",title:"Special Relativity",summary:"Einstein's theory: the laws of physics are the same for all inertial observers, and the speed of light is constant.",keyFacts:["Time dilation: moving clocks run slow: t' = t/â(1-vÂ²/cÂ²)","Length contraction: moving objects are shorter: L' = Lâ(1-vÂ²/cÂ²)","Mass-energy equivalence: E = mcÂ²","Nothing can reach or exceed the speed of light","Relativistic effects are only significant at speeds close to c","Simultaneity: events simultaneous in one frame may not be in another"],formula:"E = mcÂ²  |  t' = t/â(1-vÂ²/cÂ²)"},
    {id:"nuclear-physics",emoji:"â¢ï¸",title:"Nuclear Physics & Radioactivity",summary:"The nucleus can undergo decay, releasing particles and energy.",keyFacts:["Alpha (Î±): 2 protons + 2 neutrons â low penetration, stopped by paper","Beta (Î²): fast electron or positron â stopped by aluminium","Gamma (Î³): high-energy photon â stopped by thick lead or concrete","Half-life: time for half the nuclei in a sample to decay","Nuclear fission: heavy nucleus splits â releases enormous energy","Nuclear fusion: light nuclei combine â releases even more energy (powers stars)"]},
  ],

  // ââ VCE PSYCHOLOGY ââ
  "States of Consciousness": [
    {id:"sleep-stages",emoji:"ð´",title:"Sleep Stages & Architecture",summary:"Sleep progresses through NREM and REM stages in ~90 minute cycles.",keyFacts:["NREM Stage 1: light sleep, theta waves, easy to wake, hypnic jerks","NREM Stage 2: sleep spindles, K-complexes, harder to wake","NREM Stage 3 (SWS): delta waves, deep sleep, hardest to wake, growth hormone released","REM sleep: rapid eye movement, vivid dreams, brain as active as waking","Typical night: 4-6 cycles, REM increases in later cycles","Adults need 7-9 hours; teens need 8-10 hours"]},
    {id:"sleep-deprivation",emoji:"ðµ",title:"Sleep Deprivation Effects",summary:"Insufficient sleep impairs cognitive, physical and emotional functioning.",keyFacts:["Cognitive effects: impaired attention, memory consolidation, decision-making","Emotional effects: increased irritability, anxiety, emotional reactivity","Physical effects: weakened immune system, weight gain, cardiovascular risk","Microsleeps: involuntary 1-30 second lapses in attention â dangerous when driving","Total sleep deprivation: hallucinations after ~72 hours","Partial sleep deprivation: cumulative 'sleep debt' â can't be fully repaid quickly"]},
  ],
  "Research Methods": [
    {id:"experimental-design",emoji:"ð¬",title:"Experimental Design",summary:"The structure of a study that allows researchers to investigate cause and effect.",keyFacts:["Independent variable (IV): what the researcher manipulates","Dependent variable (DV): what is measured","Controlled variables: everything kept constant to isolate IV","Experimental group: receives the treatment/intervention","Control group: does not receive treatment â used for comparison","Random allocation: participants randomly assigned to groups â reduces bias"],},
    {id:"data-analysis-psych",emoji:"ð",title:"Data Analysis & Validity",summary:"Evaluating research quality and interpreting statistical results.",keyFacts:["Mean, median, mode: measures of central tendency","Standard deviation: measure of spread around the mean","p-value < 0.05: results are statistically significant","Validity: does the study measure what it claims to measure?","Reliability: would the study produce same results if repeated?","Generalisation: can results be applied to the broader population?"]},
  ],

  // ââ VCE LEGAL STUDIES ââ
  "Civil Law": [
    {id:"tort-law",emoji:"âï¸",title:"Tort Law â Negligence",summary:"Civil wrongs where one party's carelessness causes harm to another.",keyFacts:["Elements of negligence: duty of care, breach, damage (causation + harm)","Duty of care: legal obligation to take reasonable care (e.g. doctor-patient)","Breach: failed to meet the standard of a reasonable person","Causation: breach must have caused the damage ('but for' test)","Remoteness: damage must be a foreseeable result of the breach","Remedies: compensatory damages (to restore plaintiff to original position)"]},
    {id:"dispute-resolution",emoji:"ð¤",title:"Dispute Resolution Methods",summary:"Alternative methods to resolve civil disputes without going to court.",keyFacts:["Negotiation: parties discuss directly, no third party, cheapest","Mediation: neutral mediator helps parties reach agreement â non-binding","Conciliation: conciliator takes more active role, may suggest outcomes","Arbitration: arbitrator makes binding decision (like a private judge)","VCAT: Victorian Civil and Administrative Tribunal â quicker and cheaper than courts","Advantages of alternatives: faster, cheaper, less formal, preserves relationships"]},
  ],
  "Law Reform & Justice": [
    {id:"law-reform-process",emoji:"ð",title:"Law Reform",summary:"The process by which laws are changed to better reflect community values and needs.",keyFacts:["Reasons for reform: changing values, new technology, evidence of injustice","Parliament: can pass new legislation or amend existing laws","Courts: can develop common law through precedent (judge-made law)","Law Reform Commission: independent body that researches and recommends reforms","Royal Commissions: major investigations into serious systemic issues","Community groups, media and individual citizens can all advocate for reform"]},
    {id:"justice-principles",emoji:"ðï¸",title:"Principles of Justice",summary:"The legal system should be fair, equitable and accessible to all.",keyFacts:["Fairness: impartial hearing, evidence tested, unbiased decision-maker","Equality: all treated equally before the law","Access: barriers to the legal system (cost, language, knowledge)","Legal aid: government-funded legal assistance for those who can't afford lawyers","Interpreting services: for those with limited English","Rule of law: everyone â including government â is subject to the law"]},
  ],

  // ââ VCE HISTORY ââ
  "Revolutionary Ideas & Leaders": [
    {id:"revolutionary-ideology",emoji:"ð­",title:"Revolutionary Ideologies",summary:"The ideas and beliefs that motivated and justified revolutionary action.",keyFacts:["Liberalism: individual rights, limited government, rule of law","Republicanism: government based on popular sovereignty, not monarchy","Marxism/Socialism: class struggle, collective ownership, workers' revolution","Nationalism: self-determination, pride in national identity","Enlightenment: reason over tradition â challenged divine right of kings","Propaganda: revolutionary leaders used print, art and speeches to spread ideas"]},
    {id:"key-figures",emoji:"ð¤",title:"Key Revolutionary Leaders & Their Roles",summary:"Individuals who shaped the direction, success and character of revolutions.",keyFacts:["Leaders often emerge from educated middle class or military","Charisma and oratory: ability to inspire the masses","Competing factions: moderate vs radical revolutionaries","Role of women: often significant but frequently marginalised afterward","Leaders can be both products of revolution and drivers of its direction","Cult of personality: leaders use symbols and myths to consolidate power"]},
  ],
  "Outcomes & Significance": [
    {id:"short-long-term",emoji:"ð",title:"Short & Long-Term Outcomes",summary:"Evaluating what actually changed as a result of the revolution.",keyFacts:["Political change: new government structure, constitution, voting rights","Social change: class structure, gender roles, rights of minorities","Economic change: land ownership, industry, trade policies","Counter-revolution: attempts by conservative forces to reverse change","Short-term violence vs long-term stability â tensions between them","Did the revolution achieve its stated goals? For whom?"]},
    {id:"historiography",emoji:"ð",title:"Historiography & Historical Debate",summary:"How historians have interpreted and debated the causes and significance of revolutions.",keyFacts:["Historiography: the study of how history has been written and interpreted","Different historians emphasise: class struggle, individual leaders, ideology, economics","Revisionist historians: challenge the 'standard' interpretation with new evidence","Primary sources: documents, images, speeches from the time","Secondary sources: historians' interpretations written after the event","Evaluating sources: consider origin, purpose, value and limitation"]},
  ],

  // ââ VCE ECONOMICS ââ
  "Government Policy": [
    {id:"fiscal-policy",emoji:"ðï¸",title:"Fiscal Policy",summary:"Government use of spending and taxation to influence economic activity.",keyFacts:["Expansionary: increase spending or cut taxes â stimulate AD","Contractionary: cut spending or raise taxes â reduce inflationary pressure","Budget deficit: spending > revenue (expansionary effect)","Budget surplus: revenue > spending (contractionary effect)","Automatic stabilisers: unemployment benefits, progressive tax â work without action","Fiscal policy is slower to implement than monetary policy (political process)"]},
    {id:"monetary-policy",emoji:"ð¦",title:"Monetary Policy",summary:"Reserve Bank of Australia uses interest rates to manage inflation and growth.",keyFacts:["RBA sets the cash rate (interest rate target)","Lower cash rate: cheaper borrowing â more spending â AD increases","Higher cash rate: more expensive borrowing â less spending â inflation cools","Inflation target: RBA aims for 2-3% annual inflation","Transmission mechanism: cash rate â bank rates â borrowing/saving decisions â AD","Monetary policy is faster to implement than fiscal policy"]},
  ],
  "Labour Market": [
    {id:"unemployment",emoji:"ð·",title:"Unemployment",summary:"People who are actively seeking work but cannot find it â a key macroeconomic indicator.",keyFacts:["Unemployment rate = unemployed / labour force Ã 100","Cyclical unemployment: caused by economic downturns (demand falls)","Structural unemployment: skills mismatch â jobs change, workers don't (or can't move)","Frictional unemployment: temporary â people between jobs","Natural rate of unemployment (NAIRU): unavoidable level in a healthy economy","Underemployment: working fewer hours than desired â also a problem"],formula:"Unemployment rate = (unemployed/labour force) Ã 100"},
    {id:"wages",emoji:"ð¼",title:"Wages & Wage Determination",summary:"How wages are set in Australia and what influences them.",keyFacts:["Fair Work Commission: sets national minimum wage annually","Enterprise bargaining: employers and employees negotiate agreements","Award wages: minimum pay rates set by industry","Inflation: real wages = nominal wages adjusted for inflation","Productivity: wages tend to rise with worker productivity","Skills and education: higher skills â higher wages in most labour markets"]},
  ],

  // ââ VCE ACCOUNTING ââ
  "Recording & Reporting": [
    {id:"double-entry",emoji:"ð",title:"Double-Entry Accounting",summary:"Every transaction affects at least two accounts â debits always equal credits.",keyFacts:["Accounting equation: Assets = Liabilities + Owner's Equity","Debit: left side of account â increases assets and expenses, decreases liabilities","Credit: right side â increases liabilities and equity, decreases assets","Every journal entry: debit(s) = credit(s) (must balance)","General journal: records all transactions chronologically","General ledger: individual accounts updated from journal entries"],formula:"Assets = Liabilities + Owner's Equity"},
    {id:"trial-balance",emoji:"â",title:"Trial Balance & Error Detection",summary:"A list of all ledger accounts with their balances â checks arithmetic accuracy.",keyFacts:["Debit column total must equal credit column total","Trial balance only detects arithmetic errors â not all errors","Errors NOT detected: errors of omission (transaction not recorded at all)","Errors NOT detected: errors of commission (wrong account used)","Errors NOT detected: errors of principle (wrong type of account)","Suspense account: temporary account used while locating errors"]},
  ],
  "Financial Statements": [
    {id:"income-statement",emoji:"ð",title:"Income Statement",summary:"Shows revenue, expenses and profit for a specific accounting period.",keyFacts:["Revenue: income earned from normal business activities","COGS: Cost of Goods Sold = opening stock + purchases - closing stock","Gross Profit = Revenue - COGS","Net Profit = Gross Profit - Operating Expenses","Accrual basis: record revenue when earned, expenses when incurred","Non-cash expenses: depreciation reduces profit without cash outflow"],formula:"Gross Profit = Revenue - COGS | Net Profit = Gross Profit - Expenses"},
    {id:"balance-sheet",emoji:"âï¸",title:"Balance Sheet",summary:"Shows assets, liabilities and owner's equity at a specific point in time.",keyFacts:["Current assets: cash, receivables, inventory (converted to cash within 12 months)","Non-current assets: equipment, land, vehicles (held long-term)","Current liabilities: payables, short-term loans (due within 12 months)","Non-current liabilities: long-term loans, mortgages","Owner's equity: capital + profit - drawings","Must balance: Total Assets = Total Liabilities + Owner's Equity"]},
  ],

  // ââ VCE BUSINESS MANAGEMENT ââ
  "Human Resource Management": [
    {id:"motivation-theories",emoji:"â­",title:"Motivation Theories",summary:"Theories explaining what motivates employees and how to improve performance.",keyFacts:["Maslow's hierarchy: physiological â safety â social â esteem â self-actualisation","Herzberg: hygiene factors (prevent dissatisfaction) vs motivators (drive satisfaction)","Locke's Goal Setting: specific, challenging goals improve performance","Pink's Self-Determination: autonomy, mastery, purpose drive intrinsic motivation","Taylor's Scientific Management: financial incentives motivate workers","Apply theories: different employees may respond to different motivators"]},
    {id:"employee-relations",emoji:"ð¤",title:"Employee Relations & Workplace Disputes",summary:"Management of the relationship between employers and employees.",keyFacts:["Enterprise Bargaining Agreement (EBA): negotiated terms and conditions","Fair Work Commission: resolves workplace disputes, sets minimum wages","Industrial action: strikes, work bans, overtime bans â employee pressure tactics","Lockouts: employer prevents workers from working â employer pressure tactic","Grievance procedures: formal process for employees to raise complaints","Positive employee relations: reduces turnover, increases productivity"]},
  ],
  "Change Management": [
    {id:"lewin-force-field",emoji:"â¡",title:"Lewin's Force Field Analysis",summary:"A tool for analysing the forces driving and resisting change in an organisation.",keyFacts:["Driving forces: push for change (competition, technology, customer demand)","Restraining forces: resist change (employee resistance, cost, tradition)","If driving > restraining: change happens","Strategy: strengthen driving forces OR weaken restraining forces","Unfreeze â Change â Refreeze: Lewin's three-step change model","Large gap between current and desired state â more resistance"]},
    {id:"resistance-strategies",emoji:"ð¡ï¸",title:"Resistance to Change & Strategies",summary:"Why employees resist change and how managers can overcome resistance.",keyFacts:["Fear of the unknown: comfort with familiar routines","Loss of job security: fear of redundancy or role changes","Lack of communication: rumours fill information gaps","Strategies: communicate clearly and early, involve employees in planning","Training: build confidence with new skills and systems","Empathy: acknowledge concerns, provide support and incentives"]},
  ],

  // ââ VCE SOFTWARE DEVELOPMENT ââ
  "Programming Concepts": [
    {id:"data-structures-prog",emoji:"ð¦",title:"Data Types & Structures",summary:"The building blocks for storing and organising data in programs.",keyFacts:["Integer: whole numbers (e.g. 5, -3, 100)","Float/Real: decimal numbers (e.g. 3.14, -0.5)","String: text (e.g. 'hello', 'VCE 2025')","Boolean: True or False only","List/Array: ordered collection of items (e.g. [1, 2, 3])","Dictionary: key-value pairs (e.g. {'name': 'Veer', 'age': 17})"]},
    {id:"oop",emoji:"ð·",title:"Object-Oriented Programming",summary:"Programming paradigm that organises code into objects with attributes and methods.",keyFacts:["Class: blueprint/template for creating objects","Object: instance of a class with its own data","Attribute: data belonging to an object (e.g. name, age)","Method: function belonging to a class (behaviour of object)","Encapsulation: bundling data and methods, hiding internal details","Inheritance: child class inherits attributes/methods from parent class","Polymorphism: same method name, different behaviour in different classes"]},
    {id:"algorithms-prog",emoji:"ð",title:"Algorithms & Control Structures",summary:"Step-by-step instructions that solve problems using sequence, selection and iteration.",keyFacts:["Sequence: instructions executed in order","Selection: IF/ELIF/ELSE â executes code based on condition","Iteration: FOR loops (known count) and WHILE loops (condition-based)","Pseudocode: informal language to describe algorithm logic","Trace table: manually tracking variable values through an algorithm","Nested structures: control structures inside other control structures"]},
  ],
  "Software Development Lifecycle": [
    {id:"testing",emoji:"ð§ª",title:"Testing Strategies",summary:"Systematic methods to find and fix errors before software is released.",keyFacts:["Syntax errors: incorrect code structure (caught by compiler/interpreter)","Logic errors: code runs but gives wrong output (hardest to find)","Runtime errors: crash during execution (e.g. division by zero)","Black-box testing: tests functionality without knowing internal code","White-box testing: tests internal logic with knowledge of code","Unit testing: tests individual components in isolation","Integration testing: tests how components work together","User acceptance testing (UAT): end users test in real conditions"]},
  ],

  // ââ VCE MEDIA ââ
  "Media Codes & Conventions": [
    {id:"technical-codes",emoji:"ð¬",title:"Technical Codes",summary:"The technical choices that create meaning in media texts.",keyFacts:["Camera angles: low angle (powerful), high angle (vulnerable), eye-level (neutral)","Camera shots: extreme close-up (emotion), wide shot (context), mid shot (relationship)","Lighting: high-key (positive/happy), low-key (dark/sinister), backlighting (mystery)","Editing: cut, fade, dissolve â control pacing and mood","Sound: diegetic (within the story world) vs non-diegetic (soundtrack, voiceover)","Mise-en-scÃ¨ne: everything visible in the frame â setting, costume, props, expression"]},
    {id:"narrative-codes",emoji:"ð",title:"Narrative Codes & Genre",summary:"How stories are constructed and how genre shapes audience expectations.",keyFacts:["Narrative structure: Equilibrium â Disruption â Resolution (Todorov)","Character types: hero, villain, helper, mentor (Propp's character functions)","Genre: set of conventions audiences recognise and expect","Genre codes: iconography, settings, character types, narrative patterns","Hybridisation: combining elements of different genres","Subverting conventions: deliberately breaking genre expectations for effect"]},
  ],

  // ââ VCE DRAMA ââ
  "Drama Elements & Techniques": [
    {id:"elements-drama",emoji:"ð­",title:"Elements of Drama",summary:"The building blocks that all drama uses to create meaning and engage audiences.",keyFacts:["Role & Character: who the performer is being â layered, believable characterisation","Relationships: how characters relate to each other â power, conflict, intimacy","Tension: what drives drama â conflict, mystery, surprise, dilemma","Focus: where the audience's attention is directed","Space: how performers use the performance area â proxemics, levels, staging","Time: pace, rhythm, duration â slowing down or speeding up time in performance","Language: what is said and how (subtext is often more important than text)"]},
    {id:"voice-movement",emoji:"ð¤",title:"Voice & Movement",summary:"The performer's primary instruments for creating and communicating character.",keyFacts:["Voice: volume, pitch, pace, tone, articulation, accent, rhythm","Projection: making voice heard without shouting","Physicality: posture, gesture, facial expression, gait","Status: how body language conveys power relationships","Stillness: can be more powerful than constant movement","Eye contact: direct, averted, shared â each communicates differently"]},
  ],

  // ââ VCE MUSIC ââ
  "Music Theory & Notation": [
    {id:"pitch-scales",emoji:"ðµ",title:"Pitch, Scales & Keys",summary:"The fundamental building blocks of melody and harmony in Western music.",keyFacts:["Major scale: whole, whole, half, whole, whole, whole, half pattern (W W H W W W H)","Natural minor scale: W H W W H W W pattern","Key signature: sharps or flats at start of staff â indicates the key","Intervals: distance between two pitches (unison, 2nd, 3rd...octave)","Chromatic scale: all 12 semitones","Relative major/minor: share same key signature (e.g. C major and A minor)"]},
    {id:"rhythm-metre",emoji:"ð¥",title:"Rhythm, Metre & Notation",summary:"How music is organised in time â beat, rhythm patterns and time signatures.",keyFacts:["Beat: steady pulse underlying music","Tempo: speed of the beat (BPM = beats per minute)","Time signature: top = beats per bar, bottom = beat note value","4/4: four crotchet beats per bar (most common)","3/4: three crotchet beats (waltz feel)","Syncopation: emphasis on off-beats â creates rhythmic interest","Note values: semibreve (4), minim (2), crotchet (1), quaver (Â½), semiquaver (Â¼)"]},
    {id:"music-analysis",emoji:"ð¼",title:"Analysing Music â SHMRG",summary:"A framework for analysing and describing music systematically.",keyFacts:["S â Sound: instrumentation, timbre, texture (monophonic, homophonic, polyphonic)","H â Harmony: chords, key, major/minor, dissonance/consonance, modulation","M â Melody: shape, range, intervals, sequence, conjunct/disjunct motion","R â Rhythm: tempo, metre, syncopation, rhythmic patterns","G â Growth/Structure: form (binary AB, ternary ABA, rondo, sonata), dynamics, development","Use SHMRG as a framework for written analysis responses"]},
  ],

  // ââ VCE VISUAL COMMUNICATION DESIGN ââ
  "Design Process": [
    {id:"design-elements",emoji:"ð¨",title:"Elements & Principles of Design",summary:"The fundamental visual tools and organising principles used in all design.",keyFacts:["Elements: line, shape, form, colour, texture, tone, space","Principles: balance, contrast, emphasis/focal point, pattern, repetition, unity, movement","Colour theory: hue, saturation, value; colour wheel relationships","Typography: typeface, font weight, tracking, leading, kerning","Visual hierarchy: arrangement that guides viewer through design","Gestalt principles: proximity, similarity, closure, figure-ground"]},
    {id:"design-methods",emoji:"ð",title:"Design Methods & Drawing Techniques",summary:"Systematic processes and technical drawing skills used in design.",keyFacts:["Freehand drawing: quick idea generation, thumbnails, sketches","Instrumental drawing: precise technical drawings using tools","CAD: computer-aided design â accuracy and easy editing","Orthographic projection: front, top, side views on 2D surface","Isometric drawing: 3D representation â no vanishing points","Rendering: adding tone, texture and colour to communicate materials"]},
  ],

  // ââ VCE FOOD STUDIES ââ
  "Food Safety & Hygiene": [
    {id:"food-hazards",emoji:"â ï¸",title:"Food Hazards & HACCP",summary:"Identifying and controlling hazards to ensure food is safe to eat.",keyFacts:["Biological hazards: bacteria, viruses, parasites (most common cause of illness)","Chemical hazards: pesticides, cleaning agents, food additives","Physical hazards: glass, metal, bone, pits","HACCP: Hazard Analysis Critical Control Points â systematic food safety system","Critical control points: steps where hazards must be controlled","Temperature danger zone: 5Â°C â 60Â°C â bacteria multiply rapidly","2-hour/4-hour rule: discard if in danger zone >4 hours total"]},
    {id:"pathogens",emoji:"ð¦ ",title:"Food-Borne Illness & Pathogens",summary:"Micro-organisms that cause illness through contaminated food.",keyFacts:["Salmonella: raw poultry, eggs â causes gastroenteritis 6-72 hours after consumption","Campylobacter: undercooked chicken â most common food-borne illness in Australia","Staphylococcus aureus: food handlers with skin infections â toxin not destroyed by heat","E. coli: undercooked beef, unpasteurised juice â can cause serious kidney damage","Prevent: cook to correct temperature, separate raw/cooked, chill promptly, clean hands"]},
  ],

  // ââ VCE OUTDOOR ED ââ
  "Personal & Social Development": [
    {id:"risk-management",emoji:"â°ï¸",title:"Risk Management in Outdoor Education",summary:"Identifying, assessing and managing risks in outdoor environments.",keyFacts:["Hazard: something that could cause harm (wet rocks, weather change)","Risk: likelihood and consequence of harm occurring","Risk assessment: identify hazard â assess likelihood â assess consequence â control","Hierarchy of controls: eliminate â substitute â isolate â engineering â administrative â PPE","Dynamic risk management: ongoing assessment as conditions change","Safety vs challenge: managed risk is educational â aim for 'stretch zone' not 'panic zone'"]},
    {id:"leadership",emoji:"ð¥",title:"Leadership & Group Dynamics",summary:"How leadership and group cohesion affect outcomes in outdoor experiences.",keyFacts:["Autocratic leadership: leader decides â best in emergencies","Democratic leadership: group input â builds ownership and morale","Laissez-faire: minimal direction â works with highly skilled, motivated groups","Tuckman's stages: Forming â Storming â Norming â Performing â Adjourning","Cohesion: task cohesion (shared goal) and social cohesion (relationships)","Communication: clear, assertive, respectful â essential for group safety"]},
  ],

  // ââ VCE PE ââ
  "Energy Systems & Fatigue": [
    {id:"energy-systems-pe",emoji:"â¡",title:"Three Energy Systems",summary:"The body uses three energy systems to produce ATP â the fuel for all muscle activity.",keyFacts:["ATP-PC (phosphocreatine): 0-10 seconds, explosive power (sprint start, weightlifting), no oxygen, no lactate","Anaerobic glycolysis (lactic acid): 10 seconds-2 minutes, high intensity, produces lactate causing burning sensation","Aerobic system: 2+ minutes, low-moderate intensity, uses oxygen, COâ and HâO as by-products, most ATP produced","Energy system interplay: all three operate simultaneously, one dominates based on intensity","Oxygen uptake: VOâ max is maximum oxygen body can use â key indicator of aerobic fitness"],formula:"ATP â ADP + Pi + energy (muscle contraction)"},
    {id:"fatigue-recovery",emoji:"ð",title:"Fatigue & Recovery",summary:"Why muscles fatigue and how the body recovers after exercise.",keyFacts:["Peripheral fatigue: at the muscle â lactate accumulation, fuel depletion, ion imbalance","Central fatigue: in the brain â reduced neural drive to muscles","EPOC (excess post-exercise oxygen consumption): elevated oxygen use after exercise to restore body","Active recovery: light exercise post-training â clears lactate faster than rest","Nutrition: carbohydrates restore glycogen, protein repairs muscle","Sleep: primary time for muscle repair and adaptation (growth hormone released)"]},
  ],
  "Cardiovascular & Respiratory Adaptations": [
    {id:"acute-responses",emoji:"â¤ï¸",title:"Acute Responses to Exercise",summary:"Immediate, short-term changes in heart, lungs and blood during exercise.",keyFacts:["Heart rate: increases from ~70 bpm rest to 200+ bpm max exercise","Stroke volume: increases â more blood pumped per beat","Cardiac output = HR Ã SV: increases to deliver more Oâ to muscles","Breathing rate: increases from ~15 to 50+ breaths/min","Tidal volume: volume of air per breath increases","Blood redistribution: vasoconstriction diverts blood to working muscles"],formula:"Cardiac output = heart rate Ã stroke volume"},
    {id:"chronic-adaptations",emoji:"ðª",title:"Chronic Training Adaptations",summary:"Long-term structural and functional changes from regular training.",keyFacts:["Cardiac hypertrophy: heart wall thickens â more powerful contractions","Increased stroke volume: at rest and during exercise","Lower resting heart rate: trained athletes 40-60 bpm (more efficient)","Increased VOâ max: more oxygen delivered and used by muscles","Increased mitochondrial density: more ATP produced aerobically","Increased blood volume and haemoglobin: more oxygen-carrying capacity"]},
  ],

  // ââ VCE HEALTH & HUMAN DEVELOPMENT ââ
  "Health & Wellbeing": [
    {id:"dimensions-health",emoji:"ð",title:"Dimensions of Health & Wellbeing",summary:"Health is multidimensional â physical, social, emotional, mental and spiritual.",keyFacts:["Physical: body functioning well, fitness, disease-free","Social: quality relationships, sense of belonging, social support","Emotional: manage feelings, resilience, positive self-concept","Mental: positive thinking, effective cognitive functioning, self-esteem","Spiritual: sense of meaning, purpose and connection (may or may not involve religion)","Dimensions are interconnected â a change in one affects others"]},
    {id:"social-determinants",emoji:"ð",title:"Social Determinants of Health",summary:"Social, economic and environmental factors that significantly influence health outcomes.",keyFacts:["Income and social status: strong predictor of health â higher income = better health","Education: higher education links to better health literacy and outcomes","Social support networks: isolation linked to poorer mental and physical health","Employment and working conditions: job security, safe conditions matter","Access to healthcare: cost, location, cultural appropriateness","Early life experiences: ACEs (adverse childhood experiences) have lifelong impact"]},
  ],

  // ââ IB SUBJECTS ââ
  "Core Approaches (IB Biology)": [
    {id:"ib-bio-core",emoji:"ð¬",title:"IB Biology Core Concepts",summary:"Key themes that run through all areas of IB Biology.",keyFacts:["Cell theory: all living things are made of cells, cells come from existing cells","Metabolism: sum of all chemical reactions in an organism","Heredity: passing of genetic information from parents to offspring","Evolution: change in heritable characteristics of populations over time","Ecology: study of organisms and their interactions with the environment","Structure and function: form is correlated with function at all levels of organisation"]},
  ],
  "Paper 1 â Source Analysis": [
    {id:"source-analysis",emoji:"ð",title:"IB History Source Analysis Skills",summary:"Evaluating historical sources for their value and limitations.",keyFacts:["OPVL: Origin, Purpose, Value, Limitation","Origin: who created it, when, where â affects reliability","Purpose: why was it created â intended audience affects content","Value: what useful information does it provide for the investigation","Limitation: what does it NOT tell us, bias, incompleteness","Compare and contrast sources: identify agreements, contradictions, gaps","Primary vs secondary: both have value and limitations for historians"]},
  ],
  "Theory of Knowledge (TOK)": [
    {id:"tok-knowledge",emoji:"ð¤",title:"Knowledge & The Knower",summary:"Exploring what knowledge is, how we know things, and our role as knowers.",keyFacts:["Knowledge: justified true belief (but challenged by Gettier problems)","Personal knowledge: based on individual experience and intuition","Shared knowledge: accumulated by communities over time","Ways of knowing: reason, emotion, language, sense perception, intuition, memory, imagination, faith","Areas of knowledge: natural sciences, human sciences, history, arts, mathematics, ethics","Knowledge question: open, contestable question about the nature of knowledge itself"]},
    {id:"tok-exhibition",emoji:"ð¼ï¸",title:"TOK Exhibition",summary:"Connecting three objects to a prescribed TOK prompt â worth 33% of final grade.",keyFacts:["Choose one prompt from the IB prescribed list (released annually)","Select 3 objects (physical or digital) that connect to the prompt","Write ~950 words total â approximately 300 words per object","Explain HOW each object connects to the prompt (not just that it does)","Objects should connect to different aspects of the prompt","Real-world connection: objects must be from the real world, not hypothetical","Submit evidence of each object with the written component"]},
  ],
  "CAS (Creativity, Activity, Service)": [
    {id:"cas-requirements",emoji:"ð",title:"CAS Requirements & Learning Outcomes",summary:"CAS must be completed over the two IB years â it is pass/fail but essential.",keyFacts:["Creativity: arts, design, music, theatre, creative writing, coding â anything creative","Activity: physical exercise â sport, dance, gym, hiking, martial arts","Service: unpaid contribution to community â volunteering, tutoring, environmental projects","CAS project: at least one collaborative project combining two or three strands","7 learning outcomes must be evidenced across all CAS activities","Reflections: meaningful written reflections required â not just descriptions of activities"]},
  ],
};
...
Write ALL maths in plain text â use Â², â, Ã, Ã·, Ï â NEVER LaTeX.`;
...
Key content:
${sub.keyFacts.map(f=>`â¢ ${f}`).join("\n")}

Write ALL maths in plain text â NO LaTeX. Return ONLY valid JSON:
...
Write all maths in plain text â use Â², â, Ã, Ã·, Ï â NEVER LaTeX. Use ## headings, **bold** key terms, - bullet points.`;

      const content = await callGemini(prompt);
      localStorage.setItem(cacheKey, content);
      setDeepNotes(d => ({...d, [key]: {loading: false, visible: true, content}}));
    } catch {
      setDeepNotes(d => ({...d, [key]: {loading: false, visible: true, content: "â ï¸ Couldn't generate notes. Try again."}}));
    }
  };
  ...
  if (loadingAI) return (
    <div style={{textAlign:"center",padding:"32px",color:"var(--muted)"}}>
      ...
      <div style={{fontSize:11,marginTop:4,color:"var(--muted2)"}}>Saved after first load â instant next time</div>
  ...
  return (
    <div>
      <div style={{fontWeight:800,fontSize:13,color:"var(--muted)",textTransform:"uppercase",letterSpacing:".08em",marginBottom:12}}>
        {staticSubtopics ? "ð Key Subtopics" : "â¨ Subtopics (AI-generated from curriculum)"}
      </div>
          ...
          return (
            <div key={sub.id||i} style={{background:"var(--bg2)",border:`1px solid ${isOpen?color+"66":"var(--border)"}`,borderRadius:14,overflow:"hidden",transition:"border-color .2s"}}>
              ...
              <div style={{padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:12}}
                ...
                <div style={{color:"var(--muted)",fontSize:12,transform:isOpen?"rotate(180deg)":"none",transition:"transform .2s"}}>â¼</div>
              </div>
                ...
                <div style={{borderTop:"1px solid var(--border)"}}>
                  ...
                  <div style={{padding:"14px 18px"}}>
                    ...
                    {sub.keyFacts.map((fact,j)=>(
                      <div key={j} style={{display:"flex",gap:8,marginBottom:7,alignItems:"flex-start"}}>
                        <span style={{color:color,fontSize:13,flexShrink:0,marginTop:1}}>â¢</span>
                  ...
                  {qs?.showQuiz && (
                    <div style={{margin:"0 18px 18px",background:"var(--bg3)",borderRadius:10,padding:"14px"}}>
                      {qs.loading ? (
                        <div style={{textAlign:"center",color:"var(--muted)",fontSize:13,padding:"12px 0"}}>Generating questions... â¨</div>
                      ) : qs.done ? (
                          ...
                          {qs.questions[qs.qi]?.options?.map((opt,j)=>{
                            ...
                            return(
                              <div key={j} onClick={()=>quizChoose(sub.id,j)}
                                ...
                                {qs.answered&&isCorrect&&<span style={{float:"right"}}>â</span>}
                                {qs.answered&&isSel&&!isCorrect&&<span style={{float:"right"}}>â</span>}
                              </div>
                            );
                          })}
                          {qs.answered&&(
                            <>
                              <div style={{background:"rgba(92,224,198,.08)",border:"1px solid var(--success-bg)",borderRadius:8,padding:"9px 12px",marginTop:8,fontSize:12,color:"#9090c0"}}>
                                ð¡ {cleanMath(qs.questions[qs.qi]?.explanation)}
                              </div>
                              <button className="btn btn-p btn-sm" style={{marginTop:10}} onClick={()=>quizNext(sub.id)}>
                                {qs.qi<qs.questions.length-1?"Next â":"Finish"}
                              </button>
                            </>
                          )}
                        </div>
                      ...
                      {deepNotes[sub.id]?.loading ? (
                        ...
                        <div>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                            <div style={{fontSize:12,fontWeight:700,color:color,textTransform:"uppercase",letterSpacing:".06em"}}>ð Deep Study Notes â {sub.title}</div>
                            <button onClick={()=>setDeepNotes(d=>({...d,[sub.id]:{...d[sub.id],visible:false}}))} style={{background:"none",border:"none",color:"var(--muted)",cursor:"pointer",fontSize:14}}>â</button>
                          </div>
                  ...
                  <div style={{padding:"0 18px 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
                    <button className="btn btn-sm" style={{background:color,color:"#fff",border:"none"}} onClick={()=>loadMiniQuiz(sub)}>
                      {qs?.showQuiz?"â Hide Quiz":"ð¯ Practice Questions"}
                    </button>
                    <button className="btn btn-s btn-sm" onClick={()=>loadDeeperNotes(sub)}>
                      {deepNotes[sub.id]?.visible?"â Hide Notes":"ð Dive Deeper"}
                    </button>
                  </div>
                </div>
              )}
...
function SubjectsScreen({ profile, gs }) {
  ...
  const loadTopics = async (subject) => {
    ...
    // Alias map â Year 9/10 subjects to their curriculum entries
    const ALIASES = {
...
2. Important formulas or rules (plain text â use Â², Â³, â, Ã, Ã·, â¤, â¥, Ï, Î â NOT LaTeX)
...
- Write maths in plain text: Â², â, Ã, Ã·, Ï â NO LaTeX
...
Include:
â¢ All key terms and definitions
â¢ Important formulas/rules (plain text â Â², â, Ã, Ã·, Ï, Î â NOT LaTeX)
â¢ Key cause-effect relationships
â¢ Common exam traps to avoid
...
Use ## headings, - bullet points, **bold** for key terms. Keep it punchy and exam-focused.`
    ...
    try {
      ...
      if (!raw || !raw.trim()) {
        setContent("â ï¸ No response received. Try again.");
      ...
      setContent("â ï¸ Error generating content â " + (e.message || "try again"));
    }
    setContentLoading(false);
  };

  // ââ TOPIC STUDY VIEW â with subtopic bookmarks ââ
  if (sel && selTopic) {
    ...
    return (
      <div className="content fade-up">
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
          <button className="btn btn-g btn-sm" onClick={()=>{setSelTopic(null);setContent("");setActiveTab("overview");}}>â Back</button>
          <div style={{width:3,height:24,background:color,borderRadius:2}}/>
          <div>
            <div style={{fontWeight:900,fontSize:18}}>{selTopic}</div>
            <div style={{fontSize:12,color:"var(--muted)"}}>{sel} Â· {curriculum}</div>
          </div>
        </div>
        ...
        {topicArea?.dotPoints && (
          <div style={{background:"rgba(124,106,247,.06)",border:"1px solid rgba(124,106,247,.15)",borderRadius:12,padding:"14px 16px",marginBottom:18}}>
            <div style={{fontSize:11,fontWeight:700,color:"var(--accent)",marginBottom:8,textTransform:"uppercase",letterSpacing:".06em"}}>ð Official VCAA Curriculum</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
          {[
            {id:"study",label:"ð Full Study Notes"},
            {id:"quiz",label:"ð¯ Full Quiz (5 Qs)"},
            {id:"flashcards",label:"ð Flashcard Deck"},
            {id:"notes",label:"ð Quick Summary"},
          ].map(t=>(
        ...
        {contentLoading ? (
          ...
          <div style={{marginBottom:20}}>
            {(activeTab==="study"||activeTab==="notes") && !parsedContent && (
              <div className="card">
                <div className="ch"><div className="ct">{activeTab==="study"?"ð Study Notes":"ð Revision Summary"} â {selTopic}</div><span className="tag tag-a">{curriculum}</span></div>
    ...
    return (
      <div className="content fade-up">
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:22}}>
          <button className="btn btn-g btn-sm" onClick={()=>setSel(null)}>â Back</button>
          <div style={{width:4,height:32,background:color,borderRadius:2}}/>
          <div style={{flex:1}}>
            <div style={{fontWeight:900,fontSize:20}}>{sel}</div>
            <div style={{color:"var(--muted)",fontSize:12}}>{curriculum} Â· {subjData?.assessmentType||"Assessment"}</div>
          </div>
        ...
        <div className="card" style={{marginBottom:16,borderColor:`${color}44`}}>
          <div className="ch">
            <div className="ct">ð What are you studying right now?</div>
            ...
            {editingTopic ? (
              <div style={{display:"flex",gap:8}}>
                <input value={topicInput} onChange={e=>setTopicInput(e.target.value)}
                  ...
                  placeholder={`e.g. "Electrochemistry", "Chapter 4 â Calculus"...`}
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <span style={{fontSize:12,color:"var(--muted)"}}>â all quizzes and flashcards will focus on this</span>
              </div>
            ) : (
        ...
        <div className="card" style={{marginBottom:16}}>
          <div className="ch">
            <div className="ct">ð Your Notes / Textbook Excerpt</div>
          ...
          {editingNotes ? (
            ...
            <div className="cb">
              ...
              <div style={{fontSize:11,color:"var(--muted)",marginTop:4}}>â {myNotes.length} characters Â· Gemini will use your notes as primary source</div>
            </div>
          ) : (
            <div className="cb" style={{color:"var(--muted)",fontSize:13}}>
              Optionally paste your textbook or teacher notes â Gemini will generate questions directly from YOUR content rather than general curriculum.
            </div>
          )}
        </div>
        ...
        {topicsLoading ? (
          ...
          <>
            <div style={{fontWeight:800,fontSize:14,marginBottom:12,color:"var(--muted)",textTransform:"uppercase",letterSpacing:".06em"}}>
              {subjData?"ð Official VCAA Study Areas":"ð Topic Areas"}
            </div>
            <div className="g2" style={{marginBottom:18}}>
              {topics.map((topic,i)=>{
                      ...
                      <div style={{fontSize:12,color:"var(--muted)",lineHeight:1.5}}>
                        {area.dotPoints.slice(0,2).map((dp,j)=>(
                          <div key={j} style={{display:"flex",gap:6,marginBottom:3}}>
                            <span style={{color:color,flexShrink:0}}>â¢</span>{dp}
                          </div>
                        ))}
                        {area.dotPoints.length > 2 && <div style={{color:"var(--muted)",marginTop:3}}>+{area.dotPoints.length-2} more dot points â</div>}
                      </div>
                    )}
                    <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
                      <span className="tag tag-a">ð Notes</span>
                      <span className="tag tag-g">ð¯ Quiz</span>
                      <span className="tag tag-r">ð Flashcards</span>
                      <span style={{fontSize:10,color:"var(--muted)",padding:"2px 8px"}}>Click to study â</span>
                    </div>
                  </div>
                );
              })}
            ...
            {myTopic && (
              <div className="card" style={{borderColor:`${color}44`}}>
                <div className="ch"><div className="ct">â¡ Quick Study â {myTopic}</div><span className="tag tag-g">Current Topic</span></div>
                <div className="cb">
                  ...
                  <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                    {[
                      {type:"study",label:"ð Study Notes"},
                      {type:"quiz",label:"ð¯ Practice Quiz"},
                      {type:"flashcards",label:"ð Flashcards"},
                      {type:"notes",label:"ð Quick Summary"},
                    ].map(t=>(
    ...
    <div className="content fade-up">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div>
          ...
          <div style={{color:"var(--muted)",fontSize:13,marginTop:3}}>{subjs.length} subjects Â· {ALL_SUBJECTS[profile.yearLevel]?.label} Â· Click a subject to start studying</div>
        </div>
      </div>
              ...
              return (
                <div key={s} className="subj-card" onClick={()=>loadTopics(s)} style={{paddingTop:16}}>
                  ...
                  <div style={{marginBottom:8}}>
                    ...
                    {hasVCAA && <div style={{fontSize:9,color:"var(--muted)",marginTop:2}}>â VCAA curriculum</div>}
                  </div>
                  <div className="sn">{s}</div>
                  {myTopic && <div style={{fontSize:10,color:getColor(s),marginTop:4,fontWeight:600}}>ð {myTopic}</div>}
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// INLINE QUIZ â used inside SubjectsScreen
// âââââââââââââââââââââââââââââââââââââââââââââ
  ...
  if(done) {
    ...
    return (
      <div className="card" style={{textAlign:"center",padding:"36px 24px"}}>
        ...
        <div style={{color:"var(--muted)",marginTop:6}}>{score}/{questions.length} correct Â· +{score*60} XP</div>
        ...
        {q.options?.map((opt,i)=>{
          ...
          return (
            <div key={i} className={cls} onClick={()=>choose(i)}>
              ...
              {cleanMath(opt)}
              {answered&&i===q.correct&&<span style={{marginLeft:"auto",color:"var(--success)"}}>â</span>}
              {answered&&i===sel&&i!==q.correct&&<span style={{marginLeft:"auto",color:"var(--danger)"}}>â</span>}
            </div>
          );
        })}
        {answered&&<div className="qexp fade-up"><strong style={{color:"var(--success)"}}>ð¡ </strong>{cleanMath(q.explanation)}</div>}
        {answered&&<button className="btn btn-p" style={{marginTop:14}} onClick={next}>{qi<questions.length-1?"Next â":"Finish"}</button>}
      </div>
    </div>
  );
}
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// INLINE FLASHCARDS â used inside SubjectsScreen
// âââââââââââââââââââââââââââââââââââââââââââââ
function InlineFlashcards({ cards, subject }) {
  ...
  if(done) return (
    <div className="card" style={{textAlign:"center",padding:"36px"}}>
      ...
      <div style={{color:"var(--muted)",fontSize:14}}>Cards mastered â {subject}</div>
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <span style={{fontWeight:700,fontSize:13,color:"var(--muted)"}}>{idx+1}/{cards.length} Â· {subject}</span>
      ...
      {flipped && (
        <div style={{display:"flex",gap:12,marginTop:16}} className="fade-up">
          <button className="btn btn-full" style={{flex:1,justifyContent:"center",border:"1.5px solid rgba(255,107,107,.4)",color:"var(--danger)",background:"rgba(255,107,107,.05)"}} onClick={()=>verdict("rev")}>â Still Learning</button>
          <button className="btn btn-full" style={{flex:1,justifyContent:"center",border:"1.5px solid rgba(92,224,198,.4)",color:"var(--success)",background:"rgba(92,224,198,.05)"}} onClick={()=>verdict("know")}>â Got It!</button>
        </div>
      )}
    </div>
  );
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// QUIZ â Gemini generates questions for YOUR subjects
// âââââââââââââââââââââââââââââââââââââââââââââ
...
- Write ALL maths in plain text â use Â², Â³, â, Ã, Ã·, â¤, â¥, Ï, Î, Î¸ â NEVER use LaTeX ($...$) or backslash commands
- Example: write "â72 - â18" NOT "$\\sqrt{72} - \\sqrt{18}$"
- Example: write "xÂ² + 3x - 4" NOT "$x^2 + 3x - 4$"
...
[{"subject":"${selSubj}","question":"...","options":["A","B","C","D"],"correct":0,"explanation":"..."}]`;
      ...
      const raw = await callGemini(prompt);
      // Robust JSON extraction â handles markdown, extra text, etc.
      let clean = raw.replace(/```json\n?|```\n?/g, "").trim();
      // Find the JSON array
      const start = clean.indexOf("[");
      const end = clean.lastIndexOf("]");
      ...
      setError("Couldn't generate questions â " + (e.message || "try again"));
    }
    setLoading(false);
  };
      ...
      <div className="content fade-up">
        ...
        <div className="card">
          <div style={{textAlign:"center",padding:"48px 24px"}}>
            ...
            <div style={{fontSize:16,color:"var(--muted)",marginTop:6}}>{score}/{questions.length} correct Â· +{score*60} XP earned</div>
            <div style={{marginTop:8,fontSize:13,color:"var(--muted)"}}>{selSubj} Â· {profile.yearLevel?.toUpperCase()}</div>
            <div style={{marginTop:14,fontSize:14,color:"var(--muted)"}}>
              {pct>=75?"ð Excellent! You're on track.":pct>=50?"ð Good effort â review the missed topics.":"ð Let's revisit these with the AI Tutor."}
            </div>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center",marginTop:24}}>
              <button className="btn btn-p" onClick={restart}>New Quiz ð²</button>
    ...
    <div className="content fade-up">
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <button className="btn btn-g btn-sm" onClick={restart}>New Quiz ð²</button>
        </div>
      </div>
      ...
      {answered&&(
        <div style={{display:"flex",justifyContent:"flex-end",gap:10}} className="fade-up">
          <button className="btn btn-s">ð¤ Ask Claude</button>
          <button className="btn btn-p" onClick={next}>{qi<questions.length-1?"Next â":"Results ð¯"}</button>
        </div>
      )}
    </div>
  );
...
CRITICAL FORMATTING RULES:
- Write ALL maths in plain text â use Â², Â³, â, Ã, Ã·, â¤, â¥, Ï, Î â NEVER use LaTeX ($...$)
- Example: write "â72 - â18" NOT "$\\sqrt{72} - \\sqrt{18}$"
...
[{"q":"question","a":"answer","subject":"${selSubj}"}]`;
  ...
  if(loading) return (
    <div className="content fade-up">
      <SubjectPicker/>
      <div className="card" style={{textAlign:"center",padding:"60px 24px"}}>
        <div style={{fontSize:36,marginBottom:16}}>ð</div>
  ...
  if(done) return (
    <div className="content fade-up">
      ...
      <div className="card" style={{textAlign:"center",padding:"48px 24px"}}>
        ...
        <div style={{color:"var(--muted)",fontSize:14}}>Cards mastered â {selSubj}</div>
        <div style={{display:"flex",gap:24,justifyContent:"center",margin:"20px 0"}}>
          <div style={{textAlign:"center"}}><div style={{fontSize:24,fontWeight:900,color:"var(--success)"}}>{known.length}</div><div style={{fontSize:11,color:"var(--muted)"}}>Know it â</div></div>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <button className="btn btn-p" onClick={()=>generateCards(selSubj)}>New Deck ð²</button>
  ...
  return (
    <div className="content fade-up">
      <SubjectPicker/>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        <div><div style={{fontWeight:900,fontSize:20}}>Flashcards</div><div style={{color:"var(--muted)",fontSize:12,marginTop:3}}>Spaced Repetition Â· Tap to flip</div></div>
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <span style={{fontWeight:700,color:"var(--muted)"}}>{idx+1}/{cards.length}</span>
          <button className="btn btn-g btn-sm" onClick={()=>generateCards(selSubj)}>New Deck ð²</button>
        </div>
      </div>
      <div style={{height:5,background:"var(--bg3)",borderRadius:3,marginBottom:22,overflow:"hidden"}}>
...
Student: ${(Array.isArray(profile.selectedSubjects)?profile.selectedSubjects:[]).join(", ")} Â· Goal: ${profile.futurePath || "academic success"}
...
Rules: Australian English. Warm and direct. Plain text maths (Â², â, Ã). Bold **key terms**. If image uploaded, analyse it carefully and solve any questions shown step by step.`;
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      setMsgs(m => [...m, { role:"ai", text: "Connection error â check your internet and try again." }]);
    }
    setLoading(false);
    ...
    <div className="chat-wrap" onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
      ...
      <div className="chat-in-area">
        ...
        <div className="chat-row">
          ...
          {/* Camera â shows on mobile, useful on desktop too */}
        ...
        <div style={{fontSize:10,color:"var(--muted2)",marginTop:8,display:"flex",gap:12,alignItems:"center",justifyContent:"space-between"}}>
          <span>Powered by Google Gemini Â· {curriculum} Â· Chat saved</span>
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// STUDY PLANNER â interactive plan with sessions
// âââââââââââââââââââââââââââââââââââââââââââââ
...
- generate exactly ${dayCount} entries`;
      ...
      if (focusText.match(/sac|exam|test|assessment/)) {
        ...
        if (matchedSubj) {
          addEvent({ title:`${matchedSubj} â ${subjectFocus[matchedSubj]}`, subject:matchedSubj, date:planEnd, type:focusText.includes("sac")?"SAC":"EXAM", color:getColor(matchedSubj) });
        }
      }
    ...
    return (
      <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 56px)",background:"var(--bg)"}}>
        ...
        <div style={{padding:"14px 24px",borderBottom:"1.5px solid var(--border)",background:"var(--bg2)",display:"flex",alignItems:"center",gap:12}}>
          <button className="btn btn-ghost btn-sm" onClick={()=>{setStep("plan");setTimerRunning(false);}}>â Back</button>
          <div style={{width:34,height:34,borderRadius:"var(--r)",background:color+"22",border:`1.5px solid ${color}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>ð</div>
          <div style={{flex:1}}>
            <div style={{fontWeight:800,fontSize:14,color:"var(--text)"}}>{day.topic}</div>
            <div style={{fontSize:11,color:"var(--muted)"}}>{day.subject} Â· {day.day} {new Date(day.date+"T12:00:00").toLocaleDateString("en-AU",{day:"numeric",month:"short"})}</div>
          </div>
          <a href={`https://www.youtube.com/results?search_query=${ytQuery}`} target="_blank" rel="noreferrer"
            className="btn btn-secondary btn-sm">ð¬ Find Video</a>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
          ...
          <div style={{textAlign:"center",marginBottom:24}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              ...
              <button className="btn btn-secondary btn-sm" onClick={()=>{setTimerRunning(false);setTimer(timerMode==="work"?25*60:5*60);}}>âº Reset</button>
              <button className="btn btn-ghost btn-sm" onClick={()=>{setTimerMode(m=>m==="work"?"break":"work");setTimerRunning(false);setTimer(timerMode==="work"?5*60:25*60);}}>
                {timerMode==="work"?"â Break":"â Focus"}
              </button>
            </div>
          </div>
          ...
          {/* Tasks */}
          <div className="card">
            <div className="card-head"><div className="card-title">ð Today's Tasks</div><span style={{fontSize:12,color:"var(--muted)"}}>{tasks.filter((_,i)=>doneTasks[`${sessionDay}-${i}`]).length}/{tasks.length} done</span></div>
                ...
                return (
                  <div key={i} onClick={()=>markTask(sessionDay,i)} style={{display:"flex",alignItems:"flex-start",gap:12,padding:"12px 14px",borderRadius:"var(--r)",border:`1.5px solid ${isCurrent?"var(--text)":done?"var(--success)":"var(--border-light)"}`,background:done?"var(--success-bg)":isCurrent?"var(--bg3)":"var(--bg2)",cursor:"pointer",transition:"all .15s"}}>
                    <div style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${done?"var(--success)":isCurrent?"var(--text)":"var(--border)"}`,background:done?"var(--success)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>
                      {done&&<span style={{color:"#fff",fontSize:12,fontWeight:800}}>â</span>}
                    </div>
                    <div style={{flex:1}}>
                      ...
                      {isCurrent&&<div style={{fontSize:11,color:"var(--muted)",marginTop:3}}>â Current task</div>}
                    </div>
                  </div>
                );
              })}
      ...
      <div className="content fade-up">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button className="btn btn-primary btn-sm" onClick={()=>setStep("setup")}>â¨ Create Study Plan</button>
          </div>
        </div>

        {showAddEvent && (
          <div className="card" style={{marginBottom:16}}>
            <div className="card-head"><div className="card-title">Add Event</div><button className="btn btn-ghost btn-sm" onClick={()=>setShowAddEvent(false)}>â</button></div>
            <div className="card-body" style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        <div style={{textAlign:"center",padding:"48px 24px",background:"var(--bg2)",borderRadius:"var(--r2)",border:"1.5px dashed var(--border-light)",marginBottom:20}}>
          ...
          <button className="btn btn-primary" onClick={()=>setStep("setup")}>â¨ Create My Study Plan</button>
        </div>

        {upcoming.length > 0 && (
          <div className="card">
            <div className="card-head"><div className="card-title">ð Upcoming</div></div>
                ...
                return (
                  <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:i<Math.min(upcoming.length,5)-1?"1px solid var(--border-light)":"none"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:11,color:"var(--muted)",marginTop:1}}>{ev.subject} Â· {new Date(ev.date).toLocaleDateString("en-AU",{weekday:"short",day:"numeric",month:"short"})}</div>
                    </div>
    ...
    return (
      <div className="content fade-up">
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
          <button className="btn btn-ghost btn-sm" onClick={()=>setStep("home")}>â Back</button>
          <div style={{fontWeight:900,fontSize:20,color:"var(--text)"}}>Create Study Plan</div>
        </div>

        <div className="card" style={{marginBottom:14}}>
          <div className="card-head"><div className="card-title">ð Date Range</div></div>
          <div className="card-body">
        ...
        <div className="card" style={{marginBottom:14}}>
          <div className="card-head"><div className="card-title">ð Select Subjects</div></div>
          <div className="card-body">
        ...
        {planSubjects.length > 0 && (
          <div className="card" style={{marginBottom:14}}>
            <div className="card-head"><div className="card-title">ð¯ What to Cover</div></div>
            <div className="card-body" style={{display:"flex",flexDirection:"column",gap:12}}>
              <div style={{fontSize:13,color:"var(--muted)",marginBottom:4}}>Tell us specifically what you need to study for each subject â topics, upcoming SACs, weak areas, etc.</div>
              {planSubjects.map(s=>(
                <div key={s}>
        ...
        <button className="btn btn-primary btn-full" style={{padding:"14px"}} onClick={generatePlan}
          ...
          {generating?"â¨ Generating your plan...":"â¨ Generate My Study Plan"}
        </button>
      </div>
    );
  }
    ...
    <div className="content fade-up">
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {studyPlan.map((day, idx) => {
          ...
          const actIcon = {"Study Notes":"ð","Quiz Practice":"ð¯","Flashcards":"ð","Past Paper":"ð","Revision":"ð","Rest":"ð´"}[day.activity]||"ð";
          const ytQuery = encodeURIComponent(`${day.subject} ${day.topic} ${profile.yearLevel?.toUpperCase()||""} tutorial`);
          ...
          return (
            <div key={idx} className="card" style={{border:`1.5px solid ${isToday?"var(--gold)":done?"var(--success)":isRest?"var(--border-light)":"var(--border)"}`,opacity:done&&!isToday?0.7:1,overflow:"visible"}}>
              {/* Day header â always visible, click to expand */}
              <div onClick={()=>!isRest&&setExpanded(expanded===idx?null:idx)}
                ...
                {/* Content */}
                {isRest ? (
                  <div style={{flex:1,fontSize:14,fontWeight:600,color:"var(--muted)"}}>ð´ Rest Day</div>
                ) : (
                  <>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                        ...
                        <span style={{fontSize:10,color:"var(--muted)"}}>Â· {day.duration}min</span>
                        {day.tasks?.length>0&&<span style={{fontSize:10,color:done?"var(--success)":"var(--muted)"}}>Â· {tasksDone}/{day.tasks.length} tasks</span>}
                      </div>
                    ...
                    <div style={{display:"flex",gap:6,flexShrink:0,alignItems:"center"}}>
                      {done&&<span style={{fontSize:11,color:"var(--success)",fontWeight:800}}>â</span>}
                      <span style={{fontSize:12,color:"var(--muted)",transform:isExpanded?"rotate(180deg)":"rotate(0deg)",transition:"transform .2s"}}>â¾</span>
                    </div>
                  </>
                )}
              </div>
                        ...
                        return (
                          <div key={ti} onClick={()=>markTask(idx,ti)} style={{display:"flex",gap:10,alignItems:"flex-start",cursor:"pointer",padding:"8px 10px",borderRadius:"var(--r)",background:taskDone?"var(--success-bg)":"var(--bg2)",border:`1px solid ${taskDone?"var(--success)":"var(--border-light)"}`,transition:"all .15s"}}>
                            <div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${taskDone?"var(--success)":"var(--border)"}`,background:taskDone?"var(--success)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>
                              {taskDone&&<span style={{color:"#fff",fontSize:10,fontWeight:800}}>â</span>}
                            </div>
    ...
    <div className="content fade-up">
      ...
      <div className="g4" style={{marginBottom:18}}>
        <div className="stat" style={{background:"rgba(124,106,247,.08)",border:"1px solid rgba(124,106,247,.2)"}}>
          ...
          <div className="sl">{isIB?"Predicted IB Score":"Predicted ATAR"}</div>
          <div className="sc up">{gap!==null&&gap>0?`${gap} below target`:"On track for goal! ð¯"}</div>
        </div>
        <div className="stat">
          <div className="sv" style={{color:"#5CE0C6",fontSize:26}}>{state.xp?.toLocaleString()||0}</div>
          <div className="sl">Total XP Â· Lv {state.level||1}</div>
        ...
        <div className="stat">
          <div className="sv" style={{color:"#FF6B6B",fontSize:26}}>ð¥{state.streak||0}</div>
      ...
      {!isIB && (
        <div className="card" style={{marginBottom:18}}>
          <div className="ch">
            <div className="ct">ð¯ ATAR Goal Tracker</div>
          ...
          <div className="cb">
            <div style={{display:"flex",alignItems:"center",gap:20,marginBottom:14}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:"var(--muted)",marginTop:4}}>
                  {gap>0?`ð ${gap} ATAR points to reach your ${profile.futurePath} goal`:"ð You're on track for your goal!"}
                </div>
              </div>
              <div style={{textAlign:"center"}}>
            ...
            <div style={{fontSize:12,color:"var(--muted)",background:"var(--bg3)",borderRadius:8,padding:"10px 14px",marginBottom:12}}>
              ð¡ Your ATAR prediction improves as you complete quizzes and raise your subject mastery scores.
            </div>
            <div>
      ...
      {/* Activity chart */}
      <div className="card" style={{marginBottom:18}}>
        <div className="ch"><div className="ct">ð Study Sessions â Last 12 Weeks</div></div>
        <div className="cb">
      ...
      <div className="g2">
        <div className="card">
          <div className="ch"><div className="ct">ð Subject Mastery</div></div>
          <div className="cb">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            <div className="ch"><div className="ct">â ï¸ Focus Areas</div></div>
            <div className="cb">
              {subjMastery.length === 0 ? (
          ...
          <div className="card">
            <div className="ch"><div className="ct">ð Quiz History</div></div>
            <div style={{padding:"0 16px"}}>
              {(state.quizHistory||[]).length===0 ? (
                ...
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:i<5?"1px solid var(--border)":"none"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:10,color:"var(--muted)"}}>{q.score}/{q.total} Â· {new Date(q.date).toLocaleDateString("en-AU",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</div>
                  </div>
                </div>
              ))}
            </div>
...
function NotifToggles() {
  const NOTIFS = [
    {label:"ð SAC & Exam Countdowns",desc:"7, 3 and 1 day reminders",key:"sac"},
    {label:"ð¥ Daily Streak Reminder",desc:"Keep your streak alive",key:"streak"},
    {label:"â¡ XP Milestones",desc:"Level up and achievement alerts",key:"xp"},
    {label:"ð Weekly Summary",desc:"Sunday progress report",key:"weekly"},
  ];
...
function SettingsScreen({ profile, onUpdateProfile, onSignOut }) {
  ...
  const FUTURE_PATHS = [
    {id:"medicine",label:"ð©º Medicine / Health"},{id:"law",label:"âï¸ Law"},{id:"engineering",label:"âï¸ Engineering"},
    {id:"cs",label:"ð» Computer Science"},{id:"business",label:"ð¼ Business"},{id:"science",label:"ð¬ Science"},
    {id:"arts_hum",label:"ð¨ Arts / Humanities"},{id:"education",label:"ð Education"},{id:"trades",label:"ð§ Trades"},
  ];
  ...
  const TABS = [{id:"profile",label:"ð¤ Profile"},{id:"subjects",label:"ð Subjects"},{id:"notifications",label:"ð Alerts"},{id:"about",label:"â¹ï¸ About"}];
  ...
  return (
    <div className="content fade-up">
      ...
      {tab==="profile"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            <div className="ch"><div className="ct">ð¤ Your Profile</div><button className="btn btn-s btn-sm" onClick={()=>setEditing(e=>!e)}>{editing?"Cancel":"Edit"}</button></div>
            <div className="cb">
              <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:12,color:"var(--muted)",marginTop:4}}>{ALL_SUBJECTS[profile.yearLevel]?.label||profile.yearLevel} Â· {(Array.isArray(profile.selectedSubjects)?profile.selectedSubjects:[]).length||0} subjects</div>
                </div>
              </div>
              {!editing?(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:16}}>
                  ...
                  <button className="btn btn-p" onClick={save} style={{padding:"12px"}}>{saved?"â Saved!":"Save Changes"}</button>
                </div>
              )}
            </div>
          </div>
      ...
      {tab==="subjects"&&(
        <div className="card">
          <div className="ch"><div className="ct">ð My Subjects</div><span style={{fontSize:12,color:"var(--muted)"}}>{selectedSubjects.length}/12</span></div>
          <div className="cb">
      ...
      {tab==="notifications"&&(
        <div className="card">
          <div className="ch"><div className="ct">ð Notification Preferences</div></div>
          <div className="cb">
            <NotifToggles/>
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            <div className="cb" style={{textAlign:"center",padding:"24px"}}>
              ...
              <div style={{color:"var(--muted)",fontSize:13,marginBottom:4}}>Victorian Education Platform Â· Years 9â12</div>
              <div style={{fontSize:12,color:"var(--muted)"}}>Version 1.0.0</div>
            </div>
          </div>
          <div className="card">
            <div className="ch"><div className="ct">ð Platform Info</div></div>
            <div className="cb">
              {[
...
function SearchScreen({ profile, setScreen }) {
      ...
      data.areas?.forEach(area=>{
        ...
        area.dotPoints?.forEach(dp=>{
          if(dp.toLowerCase().includes(q))
            found.push({type:"dotpoint",title:dp.slice(0,55)+(dp.length>55?"...":""),subtitle:`${sub} â ${area.name}`,color:getColor(sub),action:"subjects"});
        });
      });
    });
    const screens=[
      {name:"dashboard",label:"Dashboard",icon:"â"},
      {name:"planner",label:"Study Planner",icon:"ð"},
      {name:"analytics",label:"Analytics",icon:"ð"},
      {name:"ai",label:"AI Tutor",icon:"â¨"},
      {name:"groups",label:"Study Groups",icon:"ð¥"},
      {name:"tutors",label:"Tutor Marketplace",icon:"ð"},
      {name:"settings",label:"Settings",icon:"âï¸"},
    ];
    screens.forEach(s=>{
  ...
  const QUICK=[
    {icon:"ð",label:"My Subjects",screen:"subjects"},
    {icon:"â¨",label:"AI Tutor",screen:"ai"},
    {icon:"ð",label:"Study Planner",screen:"planner"},
    {icon:"ð",label:"Analytics",screen:"analytics"},
    {icon:"ð¥",label:"Study Groups",screen:"groups"},
    {icon:"ð",label:"Tutor Marketplace",screen:"tutors"},
  ];
  ...
  return(
    <div className="content fade-up">
      ...
      <div style={{position:"relative",marginBottom:20}}>
        <div style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",fontSize:16,color:"var(--muted)"}}>ð</div>
        ...
        {query&&<button onClick={()=>setQuery("")} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"var(--muted)",cursor:"pointer",fontSize:18}}>â</button>}
      </div>
      {!query&&(
        <div>
          ...
          {results.map((r,i)=>(
            <button key={i} onClick={()=>setScreen(r.action)} style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:12,padding:"14px 16px",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:12,width:"100%"}}>
              <div style={{width:36,height:36,borderRadius:8,background:`${r.color}22`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>
                {{subject:"ð",topic:"ð",dotpoint:"â¢",screen:"ð"}[r.type]}
              </div>
              <div style={{flex:1}}>
              ...
              <div style={{fontSize:12,color:r.color,fontWeight:700}}>â</div>
            </button>
          ))}
        </div>
      )}
...
// âââââââââââââââââââââââââââââââââââââââââââââ
// STUDY GROUPS â full rebuild with persistence,
...
function StudyGroupsScreen({ profile, user, gs }) {
  ...
  const createGroup = async () => {
    ...
    if (!token) { setError("Not logged in â please sign out and sign back in."); return; }
  ...
  return (
    <div className="content fade-up">
      <div style={{marginBottom:20}}>
        ...
        <div style={{fontSize:13,color:"var(--muted)",marginTop:4}}>Group chat Â· XP leaderboard Â· Invite friends with a code</div>
      </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <div style={{flex:1}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,marginTop:2}}>
              ...
              {group.is_public===false && <span style={{fontSize:10,background:"var(--bg4)",color:"var(--muted)",borderRadius:20,padding:"1px 7px",fontWeight:700,border:"1px solid var(--border-light)"}}>ð Private</span>}
            </div>
          </div>
...
function GroupView({ group, profile, user, gs, isMember, onBack, onJoin }) {
  ...
  const rankIcon = i => i===0?"ð¥":i===1?"ð¥":i===2?"ð¥":`#${i+1}`;

  // Render message content â handles images embedded as [IMG:...]
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 56px)"}}>
      ...
      <div style={{padding:"10px 20px",borderBottom:"1.5px solid var(--border)",background:"var(--bg2)",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        ...
        <div style={{width:34,height:34,borderRadius:"var(--r)",background:color+"22",border:`1.5px solid ${color}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>ð</div>
        <div style={{flex:1,minWidth:0}}>
      ...
      <div style={{display:"flex",borderBottom:"1.5px solid var(--border)",background:"var(--bg2)",flexShrink:0}}>
        {[{id:"chat",label:"ð¬ Chat"},{id:"leaderboard",label:"ð Rankings"},{id:"info",label:"â¹ Info"}].map(t=>(
          ...
          <div style={{borderTop:"1.5px solid var(--border)",background:"var(--bg2)",flexShrink:0,padding:"10px 16px 12px"}}>
            {imgPreview && (
              <div style={{marginBottom:8,position:"relative",display:"inline-block"}}>
                ...
                <button onClick={()=>setImgPreview(null)} style={{position:"absolute",top:-6,right:-6,width:18,height:18,borderRadius:"50%",background:"var(--danger)",color:"#fff",border:"none",cursor:"pointer",fontSize:10,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center"}}>Ã</button>
              </div>
            )}
            ...
            <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
              <button onClick={()=>fileRef.current?.click()} style={{width:38,height:38,borderRadius:"var(--r)",border:"1.5px solid var(--border-light)",background:"var(--bg3)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>ð·</button>
      ...
      {tab==="leaderboard" && (
        <div style={{flex:1,overflowY:"auto",padding:"20px"}}>
          <div style={{fontWeight:800,fontSize:16,color:"var(--text)",marginBottom:16}}>ð {group.name} Rankings</div>
          {members.length===0 ? (
                ...
                {members.map((m,i)=>(
                  <div key={m.id||i} className={`lb-row${m.user_id===userId?" me":""}`}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:11,color:"var(--muted)"}}>Level {m.level||1} Â· {m.streak||0} streak</div>
                    </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px"}}>
          ...
          {group.code && (
            <div className="card" style={{borderColor:"var(--gold)"}}>
              <div className="card-head"><div className="card-title">ð¨ Invite Friends</div></div>
              <div className="card-body" style={{textAlign:"center"}}>
                <div style={{fontSize:34,fontWeight:900,letterSpacing:".12em",color:"var(--gold)",marginBottom:8}}>{group.code}</div>
                <div style={{fontSize:13,color:"var(--muted)",marginBottom:14,lineHeight:1.6}}>Share this code with friends â they enter it under "Join with Code" to instantly join</div>
                <button className="btn btn-primary btn-sm" onClick={copyCode}>{copied?"â Copied!":"Copy Invite Code"}</button>
              </div>
            </div>
          )}
          {group.created_by === userId && (
            <div className="card" style={{marginTop:14}}>
              <div className="card-head"><div className="card-title">ð Owner Settings</div></div>
              <div className="card-body">
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>Public Group</div>
                    <div style={{fontSize:11,color:"var(--muted)",marginTop:2}}>{group.is_public!==false?"Visible in Discover":"Hidden â invite code only"}</div>
                  </div>
  ...
  return (
    <div className="content fade-up">
      <div style={{marginBottom:20}}>
        ...
        <div style={{fontSize:13,color:"var(--muted)",marginTop:4}}>Expert tutors for your subjects Â· Sessions from $60/hr</div>
      </div>
                    ...
                    <div style={{display:"flex",gap:14,marginBottom:10}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <span style={{color:"var(--gold)",fontSize:12}}>{"â".repeat(Math.round(tutor.rating||5))}</span>
                          <span style={{fontSize:12,color:"var(--muted)",fontWeight:600}}>{tutor.rating||5} Â· {tutor.review_count||0} reviews</span>
        ...
        myProfile ? (
          <div className="card" style={{maxWidth:520}}>
            ...
            <div className="card-body">
              ...
              <div style={{fontSize:13,color:"var(--muted)"}}>${myProfile.price_per_hour}/hr Â· {myProfile.contact_email}</div>
            </div>
          </div>
        ) : (
          <div style={{textAlign:"center",padding:"60px 24px"}}>
...
function TutorDetailView({ tutor, profile, user, onBack, onReviewed }) {
  ...
  return (
    <div className="content fade-up">
      <button className="btn btn-ghost btn-sm" onClick={onBack} style={{marginBottom:20}}>â Back to tutors</button>
      <div className="card" style={{marginBottom:16}}>
        <div className="card-body">
          <div style={{display:"flex",gap:16,marginBottom:14}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{color:"var(--gold)",fontSize:14}}>{"â".repeat(Math.round(tutor.rating||5))}</div>
              <div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>{tutor.rating||5}/5 Â· {tutor.review_count||0} reviews Â· ${tutor.price_per_hour||60}/hr</div>
            </div>
          </div>
          ...
          {tutor.contact_email && tutor.available && (
            <a href={`mailto:${tutor.contact_email}?subject=Tutoring Enquiry â Study Ace&body=Hi ${tutor.name},%0A%0AI found your profile on Study Ace and would like to enquire about tutoring.%0A%0AYear level: ${profile.yearLevel?.toUpperCase()}%0ASubjects: ${(Array.isArray(profile.selectedSubjects)?profile.selectedSubjects:[]).join(", ")}%0A%0AThanks,%0A${profile.userName}`}
        ...
        <div className="card-body">
          {showReview && (
            <div style={{background:"var(--bg3)",borderRadius:"var(--r)",padding:"14px",marginBottom:14,border:"1.5px solid var(--border-light)"}}>
              <div style={{display:"flex",gap:4,marginBottom:10}}>{[1,2,3,4,5].map(n=><button key={n} onClick={()=>setRating(n)} style={{fontSize:22,background:"none",border:"none",cursor:"pointer",color:n<=rating?"var(--gold)":"var(--muted2)"}}>â</button>)}</div>
          ...
          {reviews.length===0 ? (
            <div style={{color:"var(--muted)",fontSize:13,textAlign:"center",padding:"16px 0"}}>No reviews yet â be the first!</div>
          ) : reviews.map((r,i)=>(
            <div key={r.id} style={{padding:"10px 0",borderBottom:i<reviews.length-1?"1px solid var(--border-light)":"none"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:"var(--text)"}}>{r.reviewer_name}</div><div style={{color:"var(--gold)",fontSize:11}}>{"â".repeat(r.rating)}</div></div>
...
function NotificationsPanel({ profile, gs, onClose }) {
  ...
  (state.calendarEvents||[]).filter(e=>new Date(e.date)>=new Date()).sort((a,b)=>new Date(a.date)-new Date(b.date)).slice(0,3).forEach(e=>{
    ...
    notifications.push({icon:"ð",title:`${e.title} in ${days} day${days!==1?"s":""}`,subtitle:`${e.subject} Â· ${new Date(e.date).toLocaleDateString("en-AU",{month:"short",day:"numeric"})}`,color:days<=3?"var(--danger)":days<=7?"var(--gold)":"var(--success)",time:"Upcoming"});
  });
  if(state.streak>0) notifications.push({icon:"ð¥",title:`${state.streak} day streak! Keep it up!`,subtitle:"Study today to maintain your streak",color:"#FF6B6B",time:"Today"});
  if(state.xp>=100) notifications.push({icon:"â¡",title:`Level ${state.level} â ${500-(state.xp%500)} XP to next level`,subtitle:"Keep studying to level up",color:"var(--accent)",time:"Progress"});
  const weakSubjects=(Array.isArray(profile.selectedSubjects)?profile.selectedSubjects:[]).filter(s=>(state.masteryMap?.[s]||50)<60);
  if(weakSubjects?.length>0) notifications.push({icon:"â ï¸",title:`${weakSubjects[0]} needs attention`,subtitle:`Mastery at ${state.masteryMap?.[weakSubjects[0]]||50}% â take a quiz`,color:"var(--gold)",time:"Study tip"});
  if(notifications.length===0) notifications.push({icon:"â",title:"You're all caught up!",subtitle:"No new notifications",color:"var(--success)",time:"Now"});
  ...
  return(
    <div style={{position:"absolute",top:65,right:16,width:320,background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:16,boxShadow:"0 8px 32px rgba(0,0,0,.4)",zIndex:1000,overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"14px 16px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{fontWeight:800,fontSize:15}}>ð Notifications</div>
        <button onClick={onClose} style={{background:"none",border:"none",color:"var(--muted)",cursor:"pointer",fontSize:16}}>â</button>
      </div>
...
export default function App() {
  ...
  // ââ On mount: check for existing session or OAuth callback ââ
  useEffect(() => {
    const init = async () => {
      try {
        const toArray = (v) => {
  ...
  const handleAuth = async (u) => {
    setUser(u);
    // Check localStorage first â instant
    const localProfile = (() => {
      try { return JSON.parse(localStorage.getItem("ss_profile") || "null"); } catch { return null; }
    })();
  ...
  const handleOnboard = async (data) => {
    ...
    // Save to localStorage immediately â fastest
  ...
  const NAV = [
    {id:"dashboard",icon:"â",label:"Dashboard",section:"learn"},
    {id:"subjects",icon:"ð",label:"Subjects",section:"learn"},
    {id:"ai",icon:"â¨",label:"AI Tutor",badge:"FREE",section:"learn"},
    {id:"planner",icon:"ð",label:"Study Planner",section:"track"},
    {id:"analytics",icon:"ð",label:"Analytics",section:"track"},
    {id:"groups",icon:"ð¥",label:"Study Groups",section:"community"},
    {id:"tutors",icon:"ð",label:"Tutor Marketplace",section:"community"},
    {id:"search",icon:"ð",label:"Search",section:"hidden"},
    {id:"settings",icon:"âï¸",label:"Settings",section:"hidden"},
  ];
      ...
      <div className="app">
        ...
        <div className="sidebar">
          <div className="logo">
            ...
            <div>
              ...
              <span className="logo-sub">VCE Â· IB Â· Years 9â12</span>
            </div>
          </div>
          ...
          <div className="sb-bottom">
            <div className="xp-row">
              <div className="xp-lbl"><span>Level {gs.state.level||1} Â· {(gs.state.xp||0).toLocaleString()} XP</span><span>{500-((gs.state.xp||0)%500)} to next</span></div>
            ...
            <button className="user-row" onClick={()=>setScreen("settings")} style={{width:"100%",background:"none",border:"1.5px solid transparent",cursor:"pointer",display:"flex",alignItems:"center",gap:9,padding:8,borderRadius:"var(--r)",transition:"all .15s",fontFamily:"var(--ff)"}}>
              ...
              <div style={{flex:1,minWidth:0,textAlign:"left"}}>
                ...
                <div className="u-sub">{yearLabel} Â· {user?.provider === "google" ? "Google" : "Email"}</div>
              </div>
        ...
        <div className="main" onClick={()=>showNotifications&&setShowNotifications(false)}>
          <div className="topbar">
            ...
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <div className="chip chip-fire">ð¥ {gs.state.streak||0}</div>
              <div className="chip chip-xp">â¡ {(gs.state.xp||0).toLocaleString()}</div>
              ...
              <button className="icon-btn" onClick={()=>setScreen("search")} style={{background:"none",border:"1.5px solid var(--border-light)",cursor:"pointer",fontFamily:"var(--ff)",fontSize:15}}>ð</button>
            </div>
          </div>
          {showNotifications && (
        ...
        <nav className="mobile-nav">
          {[
            ...
            {id:"subjects",icon:"ð",label:"Subjects"},
            {id:"ai",icon:"â¦",label:"AI Tutor"},
            {id:"groups",icon:"ð¥",label:"Groups"},
