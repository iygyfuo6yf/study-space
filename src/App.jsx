...
function Dashboard({ profile, setScreen }) {
  const heat = Array.from({length:119},()=>Math.floor(Math.random()*5));
  const upcoming = [
    {type:"SAC",subj:profile.selectedSubjects[0]||"English",topic:"SAC Assessment",date:"May 14",days:5,color:getColor(profile.selectedSubjects[0])},
    {type:"EXAM",subj:profile.selectedSubjects[1]||"Maths",topic:"Practice Exam",date:"May 19",days:10,color:getColor(profile.selectedSubjects[1])},
    {type:"SAC",subj:profile.selectedSubjects[2]||"Science",topic:"Lab Report SAC",date:"May 23",days:14,color:getColor(profile.selectedSubjects[2])},
  ];
  const leaderboard = [
    {rank:1,name:"Priya M.",xp:4820,avatar:"P"},{rank:2,name:"James L.",xp:4510,avatar:"J"},
    {rank:3,name:"You",xp:4280,avatar:"Y",me:true},{rank:4,name:"Sophie K.",xp:3990,avatar:"S"},
    {rank:5,name:"Aiden T.",xp:3740,avatar:"A"},
  ];
  ...
  return (
    <div className="content fade-up">
      ...
      <div style={{marginBottom:20}}>
        <div style={{fontFamily:"var(--ff)",fontSize:22,fontWeight:900}}>
          Good morning, {profile.userName?.split(" ")[0]} 👋
        </div>
        <div style={{color:"#6060a0",fontSize:13,marginTop:3}}>
          {profile.yearLevel === "vce" ? "VCE" : profile.yearLevel === "ib" ? "IB Diploma" : ALL_SUBJECTS[profile.yearLevel]?.label} · {profile.selectedSubjects.length} subjects · {new Date().toLocaleDateString("en-AU",{weekday:"long",month:"long",day:"numeric"})}
        </div>
      </div>
      ...
      <div className="g4" style={{marginBottom:18}}>
        {[
          {val: profile.yearLevel==="ib"?"38/45":"87.00", label: profile.yearLevel==="ib"?"Predicted IB Score":"Predicted ATAR", color:"#7C6AF7", change:"↑ +2.5 this month"},
          {val:"4,280",label:"Total XP",color:"#5CE0C6",change:"↑ +320 this week"},
          {val:"🔥 7",label:"Day Streak",color:"#FF6B6B",change:"Personal best: 12"},
          {val:"24h",label:"Study Time (7d)",color:"#FFD700",change:"↑ +4h vs last week"},
        ].map((s,i)=>(
          <div key={i} className="stat">
            <div className="sv" style={{color:s.color}}>{s.val}</div>
            <div className="sl">{s.label}</div>
            <div className="sc up">{s.change}</div>
          </div>
        ))}
      </div>
      ...
      <div className="g2" style={{marginBottom:18}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {profile.selectedSubjects.slice(0,4).map(s=>(
              <div key={s} className="subj-card" onClick={()=>setScreen("subjects")} style={{paddingTop:20}}>
                ...
                <div className="mb"><div className="mf" style={{width:`${50+Math.random()*40}%`,background:getColor(s)}}/></div>
                <div className="ml"><span>Mastery</span><span style={{color:getColor(s),fontWeight:700}}>{Math.floor(50+Math.random()*45)}%</span></div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            <div className="ch"><div className="ct">📅 Upcoming Assessments</div><span className="cl" onClick={()=>setScreen("planner")}>Planner →</span></div>
            <div style={{padding:"0 16px"}}>
              {upcoming.map((u,i)=>(
                <div key={i} className="up-item">
                  <div className="up-dot" style={{background:u.color}}/>
                  <div className="up-info">
                    <div className="up-name">{u.topic}</div>
                    <div className="up-sub"><span className={`tag ${u.type==="SAC"?"tag-a":"tag-r"}`}>{u.type}</span> {u.subj}</div>
                  </div>
                  <div className="up-days" style={{color:u.days<=5?"var(--a3)":u.days<=12?"var(--gold)":"var(--a2)"}}>{u.days}d<div style={{fontSize:10,color:"#50508a",fontWeight:400}}>{u.date}</div></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="card" style={{marginBottom:18}}>
        ...
        <div className="cb"><div className="heat">{heat.map((v,i)=><div key={i} className={`hc h${v}`}/>)}</div></div>
      </div>

      {/* Bottom */}
      <div className="g2">
        <div className="card">
          <div className="ch"><div className="ct">🏅 Achievements</div><span className="cl">All →</span></div>
          <div className="cb">
            {[{icon:"🔥",name:"7-Day Streak",desc:"Studied 7 days in a row",e:true},{icon:"⚡",name:"Quiz Master",desc:"100% on 5 quizzes",e:true},{icon:"📚",name:"Bookworm",desc:"50 flashcard reviews",e:true},{icon:"🎯",name:"Sharp Shooter",desc:"15 correct in a row",e:false}].map((a,i)=>(
        ...
        <div className="card">
          <div className="ch"><div className="ct">🏆 Class Leaderboard</div><span className="cl">Full →</span></div>
          <div className="cb">
            {leaderboard.map((l,i)=>(
              <div key={i} className={`lb-row${l.me?" me":""}`}>
                <div className={`lb-rank${i===0?" gold":i===1?" silver":i===2?" bronze":""}`}>{i===0?"🥇":i===1?"🥈":i===2?"🥉":`#${l.rank}`}</div>
                <div className="av" style={{width:26,height:26,fontSize:10}}>{l.avatar}</div>
                <div style={{flex:1,fontSize:13,fontWeight:600}}>{l.name}{l.me&&<span className="tag tag-a" style={{marginLeft:6}}>You</span>}</div>
                <div style={{fontSize:12,fontWeight:700,color:"var(--accent)"}}>{l.xp.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
...
function SubjectsScreen({ profile }) {
  ...
  if (sel) {
    ...
    const mastery = Math.floor(50+Math.random()*45);
    const topics = {
      "Maths Methods":["Functions & Graphs","Differentiation","Integration","Probability","Algebra & Equations"],
      "Chemistry":["Organic Chemistry","Electrochemistry","Energetics","Equilibrium","Acids & Bases"],
      "Physics":["Motion & Forces","Electricity","Waves & Light","Thermodynamics","Modern Physics"],
      "English":["Analytical Essay","Comparative Analysis","Reading & Creating","Metalanguage","Oral Presentation"],
      "Biology":["Cell Biology","Genetics","Evolution","Ecology","Human Physiology"],
      "Psychology":["Research Methods","Neuroscience","Learning","Memory","Mental Wellbeing"],
      "default":["Core Concepts","Key Skills","Assessment Prep","Past Paper Practice","Revision Summary"]
    };
    const t = topics[sel] || topics.default;
    return (
      <div className="content fade-up">
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:22}}>
          ...
          <div>
            ...
            <div style={{color:"#6060a0",fontSize:12}}>{profile.yearLevel?.toUpperCase()} · Units 3 & 4</div>
          </div>
          ...
          {t.map((topic,i)=>(
            <div key={i} className="card" style={{padding:18,cursor:"pointer",borderTop:`3px solid ${color}`}}>
              ...
              <div className="mb" style={{height:6}}>
                <div className="mf" style={{width:`${35+Math.random()*60}%`,background:color}}/>
              </div>
            </div>
          ))}
        </div>
        <div className="card">
          ...
          <div className="cb">
            <div className="g3">
              {["SAC Practice","Exam Questions","Past Papers","Study Guide","Video Lessons","Ask Claude AI"].map((r,i)=>(
        ...
        <div key={group} style={{marginBottom:24}}>
          ...
          <div className="g3">
            {ss.map(s=>{
              ...
              return (
                <div key={s} className="subj-card" onClick={()=>setSel(s)} style={{paddingTop:20}}>
                  <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:getColor(s)}}/>
...
// ─────────────────────────────────────────────
const QUIZ_Q = [
  {subj:"Chemistry",q:"Which best describes a nucleophilic substitution reaction?",opts:["Electron-rich species attacks electron-deficient carbon","Free radical attacks a double bond","Acid donates proton to a base","Electrophile bonds to a nucleophile"],ans:0,exp:"SN1/SN2 reactions involve a nucleophile (electron-rich) attacking an electrophilic carbon, displacing a leaving group."},
  {subj:"Maths Methods",q:"If f(x) = 3x⁴ − 2x² + 5x, what is f′(x)?",opts:["12x³ − 4x + 5","12x³ − 2x + 5","3x³ − 4x + 5","12x⁴ − 4x + 5"],ans:0,exp:"Power rule: d/dx(3x⁴)=12x³, d/dx(−2x²)=−4x, d/dx(5x)=5. So f′(x)=12x³−4x+5."},
  {subj:"Psychology",q:"Which sleep stage is most associated with memory consolidation?",opts:["Stage 1 NREM","Stage 2 NREM","Stage 3 (Slow-Wave)","REM sleep"],ans:3,exp:"REM sleep is critical for procedural and emotional memory consolidation. Slow-wave consolidates declarative memory."},
  {subj:"Physics",q:"A 2 kg object accelerates at 4 m/s². Net force?",opts:["2 N","8 N","0.5 N","6 N"],ans:1,exp:"F = ma = 2×4 = 8 N (Newton's Second Law)."},
  {subj:"Biology",q:"What process converts glucose to pyruvate in the cytoplasm?",opts:["Krebs cycle","Oxidative phosphorylation","Glycolysis","Calvin cycle"],ans:2,exp:"Glycolysis splits glucose (6C) into 2 pyruvate (3C) molecules in the cytoplasm, producing 2 ATP net."},
  {subj:"Legal Studies",q:"Which court has unlimited civil and criminal jurisdiction in Victoria?",opts:["Magistrates Court","County Court","Supreme Court","Children's Court"],ans:2,exp:"The Supreme Court of Victoria has unlimited jurisdiction and is the highest court in the state, hearing serious criminal and civil matters."},
  {subj:"History",q:"What event triggered the outbreak of World War I?",opts:["Invasion of Poland","Assassination of Archduke Franz Ferdinand","Sinking of the Lusitania","Russian Revolution"],ans:1,exp:"The assassination of Archduke Franz Ferdinand in Sarajevo (June 28, 1914) triggered the chain of alliances that led to WWI."},
  {subj:"English",q:"What is the term for attributing human qualities to non-human things?",opts:["Metaphor","Simile","Personification","Alliteration"],ans:2,exp:"Personification gives human characteristics to non-human objects or abstract ideas, e.g. 'The wind whispered.'"},
];

function QuizScreen({ profile }) {
  ...
  const next = () => {
    if (qi < QUIZ_Q.length-1) { setQi(i=>i+1); setSel(null); setAnswered(false); }
    else setDone(true);
  };
  const restart = () => { setQi(0); setSel(null); setAnswered(false); setScore(0); setDone(false); setResults([]); };
      ...
      <div className="content fade-up">
        <div className="card">
          <div style={{textAlign:"center",padding:"48px 24px"}}>
            ...
            <div style={{fontSize:16,color:"#7070a8",marginTop:6}}>{score}/{QUIZ_Q.length} correct · +{score*60} XP earned</div>
            <div style={{marginTop:14,fontSize:14,color:"#9090b8"}}>
              {pct>=75?"🎉 Excellent! You're on track.":pct>=50?"👍 Good effort — review missed topics.":"📚 Let's revisit with Claude AI Tutor."}
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:16}}>
              {results.map((r,i)=><div key={i} style={{width:11,height:11,borderRadius:"50%",background:r.ok?"var(--a2)":"var(--a3)"}}/>)}
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"center",marginTop:24}}>
              <button className="btn btn-p" onClick={restart}>Try Again</button>
              <button className="btn btn-s">📊 Analyse Mistakes</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="content fade-up">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        <div>
          ...
          <span className="tag tag-a">{q.subj}</span>
        </div>
        <div style={{fontWeight:700,color:"#6060a0"}}>Q {qi+1} / {QUIZ_Q.length}</div>
      </div>
      <div style={{height:5,background:"var(--bg3)",borderRadius:3,marginBottom:22,overflow:"hidden"}}>
        <div style={{height:"100%",background:"linear-gradient(90deg,#7C6AF7,#5CE0C6)",width:`${((qi+(answered?1:0))/QUIZ_Q.length)*100}%`,transition:"width .3s ease",borderRadius:3}}/>
      </div>
      <div className="card" style={{marginBottom:14}}>
        <div className="cb">
          <div style={{fontWeight:700,fontSize:16,lineHeight:1.5,marginBottom:22}}>{q.q}</div>
          {q.opts.map((opt,i)=>{
            let cls="qopt";
            if(answered){cls+=" qdis";if(i===q.ans)cls+=" qcor";else if(i===sel)cls+=" qwrg";}
            return (
              <div key={i} className={cls} onClick={()=>choose(i)}>
                <div className="ql" style={answered&&i===q.ans?{background:"rgba(92,224,198,.2)",color:"var(--a2)"}:answered&&i===sel?{background:"rgba(255,107,107,.2)",color:"var(--a3)"}:{}}>{String.fromCharCode(65+i)}</div>
                {opt}
                {answered&&i===q.ans&&<span style={{marginLeft:"auto",color:"var(--a2)"}}>✓</span>}
                {answered&&i===sel&&i!==q.ans&&<span style={{marginLeft:"auto",color:"var(--a3)"}}>✗</span>}
              </div>
            );
          })}
          {answered&&<div className="qexp fade-up"><strong style={{color:"var(--a2)"}}>💡 </strong>{q.exp}</div>}
        </div>
      </div>
      {answered&&(
        <div style={{display:"flex",justifyContent:"flex-end",gap:10}} className="fade-up">
          <button className="btn btn-s">🤖 Ask Claude</button>
          <button className="btn btn-p" onClick={next}>{qi<QUIZ_Q.length-1?"Next →":"Results 🎯"}</button>
        </div>
      )}
    </div>
  );
...
// ─────────────────────────────────────────────
const FC_CARDS = [
  {q:"Le Chatelier's Principle?",a:"When equilibrium is disturbed, the system shifts to counteract the change and re-establish equilibrium.",subj:"Chemistry"},
  {q:"Chain rule in calculus?",a:"If y = f(g(x)), then dy/dx = f′(g(x)) · g′(x). Used for composite functions.",subj:"Maths Methods"},
  {q:"Classical vs operant conditioning?",a:"Classical: pairs neutral stimulus with unconditioned stimulus. Operant: uses rewards/punishment to modify voluntary behaviour.",subj:"Psychology"},
  {q:"Faraday's Law of Induction?",a:"Induced EMF is proportional to rate of change of magnetic flux: ε = −dΦ/dt",subj:"Physics"},
  {q:"Glycolysis — where & product?",a:"Occurs in cytoplasm. Splits glucose (6C) into 2 pyruvate (3C), producing net 2 ATP.",subj:"Biology"},
  {q:"What is separation of powers?",a:"Constitutional principle dividing government into Legislature (makes laws), Executive (implements) and Judiciary (interprets).",subj:"Legal Studies"},
  {q:"Integration by parts formula?",a:"∫u dv = uv − ∫v du. Used when integrand is a product of two functions.",subj:"Maths Methods"},
  {q:"Causes of Russian Revolution (1917)?",a:"WWI strain, food shortages, Tsar's incompetence, rising socialist movements, Bloody Sunday legacy, February & October revolutions.",subj:"History"},
  {q:"What is metalanguage in English?",a:"Language used to describe language itself — e.g. 'symbolism', 'juxtaposition', 'tone', 'syntax', 'metaphor'.",subj:"English"},
  {q:"Hess's Law?",a:"Total enthalpy change for a reaction is independent of the pathway taken — only depends on initial and final states. ΔH = ΣΔHproducts − ΣΔHreactants",subj:"Chemistry"},
];
...
function FlashcardsScreen() {
  ...
  const [done, setDone] = useState(false);
  const card = FC_CARDS[idx];

  const verdict = (v) => {
    if(v==="know") setKnown(k=>[...k,idx]); else setRev(r=>[...r,idx]);
    setFlipped(false);
    setTimeout(()=>{ if(idx<FC_CARDS.length-1) setIdx(i=>i+1); else setDone(true); },220);
  };
  ...
  if(done) return (
    <div className="content fade-up">
      <div className="card" style={{textAlign:"center",padding:"48px 24px"}}>
        ...
        <div style={{fontWeight:900,fontSize:52,color:"var(--a2)",margin:"12px 0"}}>{known.length}/{FC_CARDS.length}</div>
        <div style={{color:"#7070a8",fontSize:14}}>Cards mastered</div>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <button className="btn btn-p" onClick={()=>{setIdx(0);setFlipped(false);setKnown([]);setRev([]);setDone(false);}}>Restart</button>
          <button className="btn btn-s">📅 Schedule Review</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="content fade-up">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        ...
        <div style={{fontWeight:700,color:"#6060a0"}}>{idx+1}/{FC_CARDS.length}</div>
      </div>
      <div style={{height:5,background:"var(--bg3)",borderRadius:3,marginBottom:22,overflow:"hidden"}}>
        <div style={{height:"100%",background:"linear-gradient(90deg,#7C6AF7,#5CE0C6)",width:`${(idx/FC_CARDS.length)*100}%`,transition:"width .3s",borderRadius:3}}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 220px",gap:16}}>
          ...
          <div className="fc-wrap" onClick={()=>setFlipped(f=>!f)}>
            <div className={`fc-inner${flipped?" flip":""}`} style={{minHeight:220}}>
              <div className="fc-face fc-front">
                <div className="fc-sub">{card.subj}</div>
        ...
        <div className="card">
          <div className="ch" style={{padding:"12px 14px"}}><div className="ct" style={{fontSize:13}}>All Decks</div></div>
          <div style={{padding:"8px 14px"}}>
            {["Chemistry","Maths Methods","Psychology","Physics","Biology","History"].map((d,i)=>(
              <div key={i} style={{padding:"8px 0",borderBottom:i<5?"1px solid var(--border)":"none",cursor:"pointer"}}>
                <div style={{fontSize:12,fontWeight:700,marginBottom:2,color:getColor(d)}}>{d}</div>
                <div style={{fontSize:10,color:"#50508a"}}>{Math.floor(20+Math.random()*25)} cards · {Math.floor(3+Math.random()*15)} due</div>
              </div>
            ))}
          </div>
        </div>
...
// PLANNER
// ─────────────────────────────────────────────
function PlannerScreen({ profile }) {
  const days = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"];
  const dates = [9,10,11,12,13,14,15];
  const evMap = {
    0:[{t:profile.selectedSubjects?.[0]||"English",c:getColor(profile.selectedSubjects?.[0])},{t:profile.selectedSubjects?.[1]||"Maths",c:getColor(profile.selectedSubjects?.[1])}],
    1:[{t:profile.selectedSubjects?.[2]||"Science",c:getColor(profile.selectedSubjects?.[2])}],
    2:[{t:"SAC Prep",c:"#7C6AF7"}],
    3:[{t:profile.selectedSubjects?.[0]||"English",c:getColor(profile.selectedSubjects?.[0])}],
    4:[{t:"Past Papers",c:"#F7971E"},{t:"Flashcards",c:"#43C6AC"}],
    5:[{t:"Full Revision",c:"#FFD700"}],
    6:[],
  };
  return (
    <div className="content fade-up">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div><div style={{fontWeight:900,fontSize:22}}>Study Planner</div><div style={{color:"#6060a0",fontSize:13,marginTop:3}}>Week of May 9 – 15, 2025</div></div>
        <div style={{display:"flex",gap:8}}>
          <button className="btn btn-s btn-sm">← Prev</button>
          <button className="btn btn-s btn-sm">Next →</button>
          <button className="btn btn-p btn-sm">🤖 AI Generate Plan</button>
        </div>
      </div>
      <div className="card" style={{marginBottom:18}}>
        <div className="cb">
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:8}}>
            {days.map((d,i)=>(
              <div key={i} className={`plan-day${i===0?" today":""}`}>
                <div className="plan-dl">{d}</div>
                <div className="plan-dn">{dates[i]}</div>
                {(evMap[i]||[]).map((e,j)=>(
                  <div key={j} className="plan-ev" style={{background:`${e.c}22`,color:e.c}}>{e.t}</div>
                ))}
              </div>
            ))}
          </div>
      ...
      <div className="g2">
        <div className="card">
          <div className="ch"><div className="ct">📋 Today's Schedule</div><button className="btn btn-g btn-sm">+ Add</button></div>
          <div>
            {profile.selectedSubjects?.slice(0,4).map((s,i)=>(
              <div key={i} style={{display:"flex",gap:12,padding:"12px 18px",borderBottom:i<3?"1px solid var(--border)":"none"}}>
                <div style={{textAlign:"right",minWidth:55}}>
                  <div style={{fontSize:11,fontWeight:700,color:"#50508a"}}>{["9:00 AM","11:00 AM","2:00 PM","4:00 PM"][i]}</div>
                  <div style={{fontSize:10,color:"#40406a"}}>{["90 min","60 min","45 min","60 min"][i]}</div>
                </div>
                <div style={{width:3,background:getColor(s),borderRadius:2,flexShrink:0}}/>
                <div>
                  <div style={{fontSize:13,fontWeight:700}}>{s} Study Session</div>
                  <div style={{fontSize:11,color:"#6060a0",marginTop:2}}>Focus: Key concepts & practice</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            ...
            <div className="cb" style={{textAlign:"center"}}>
              <div style={{fontWeight:900,fontSize:52,color:"var(--accent)",letterSpacing:-2,fontFamily:"var(--ff)"}}>25:00</div>
              <div style={{color:"#6060a0",fontSize:12,marginBottom:14}}>Focus Session</div>
              <div style={{display:"flex",gap:8,justifyContent:"center"}}>
                <button className="btn btn-p btn-sm">▶ Start</button>
                <button className="btn btn-g btn-sm">↺ Reset</button>
              </div>
            </div>
          </div>
          <div className="card">
            <div className="ch"><div className="ct">✅ Daily Goals</div></div>
            <div className="cb">
              {profile.selectedSubjects?.slice(0,3).map((s,i)=>(
                <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"7px 0",borderBottom:i<2?"1px solid var(--border)":"none"}}>
                  ...
                  <span style={{fontSize:12}}>Study {s} — 45 mins</span>
                </div>
              ))}
            </div>
          </div>
...
// ANALYTICS
// ─────────────────────────────────────────────
function AnalyticsScreen({ profile }) {
  const bars = [65,72,68,80,75,85,82,79,88,84,87,90];
  ...
  return (
    <div className="content fade-up">
      ...
      <div className="g4" style={{marginBottom:18}}>
        {[
          {val:isIB?"38/45":"87.00",label:isIB?"Predicted IB Score":"Predicted ATAR",color:"#7C6AF7"},
          {val:"214h",label:"Total Study Hours",color:"#5CE0C6"},
          {val:"78%",label:"Quiz Accuracy",color:"#FFD700"},
          {val:"1,840",label:"Flashcards Reviewed",color:"#FF6B6B"},
        ].map((s,i)=>(
          <div key={i} className="stat">
            <div className="sv" style={{color:s.color,fontSize:24}}>{s.val}</div>
            <div className="sl">{s.label}</div>
            <div className="sc up">↑ Improving</div>
          </div>
        ))}
      </div>
      <div className="card" style={{marginBottom:18}}>
        <div className="ch"><div className="ct">📈 {isIB?"IB Score":"ATAR"} Prediction Trend</div><span className="tag tag-g">On Track</span></div>
        <div className="cb">
          <div style={{display:"flex",alignItems:"flex-end",gap:7,height:150}}>
            {bars.map((b,i)=>(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                <div style={{fontSize:9,color:"#50508a",fontFamily:"var(--ff)"}}>{b}</div>
                <div style={{width:"100%",background:`linear-gradient(180deg,#7C6AF7,#5CE0C6)`,borderRadius:"3px 3px 0 0",height:`${(b/mx)*125}px`,transition:"height 1s ease",opacity:i===bars.length-1?1:.55}}/>
                <div style={{fontSize:9,color:"#40406a"}}>{months[i]}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="g2">
        <div className="card">
          ...
          <div className="cb">
            {profile.selectedSubjects?.slice(0,6).map(s=>{
              const m = Math.floor(45+Math.random()*50);
              return (
                <div key={s} className="mast-row">
                  <div className="mast-hd"><span>{s}</span><span style={{color:getColor(s),fontWeight:700}}>{m}%</span></div>
                  <div className="mb" style={{height:7}}><div className="mf" style={{width:`${m}%`,background:getColor(s)}}/></div>
                </div>
              );
            })}
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            ...
            <div className="cb">
              {profile.selectedSubjects?.slice(0,3).map((s,i)=>{
                const m=Math.floor(35+i*8);
                return(
                  <div key={s} style={{marginBottom:12}}>
                    <div className="mast-hd"><span style={{fontSize:12}}>{s}</span><span style={{fontSize:12,fontWeight:700,color:"var(--a3)"}}>{m}%</span></div>
                    <div className="mb" style={{height:6}}><div className="mf" style={{width:`${m}%`,background:"var(--a3)"}}/></div>
                  </div>
                );
              })}
              <button className="btn btn-p btn-sm btn-full" style={{marginTop:4}}>🤖 AI Study Plan</button>
            </div>
          </div>
          <div className="card">
            <div className="ch"><div className="ct">🏆 Leaderboard</div></div>
            <div style={{padding:"8px 14px"}}>
              {[{n:"Priya M.",x:4820,av:"P"},{n:"James L.",x:4510,av:"J"},{n:"You",x:4280,av:"Y",me:true},{n:"Sophie K.",x:3990,av:"S"}].map((l,i)=>(
                <div key={i} className={`lb-row${l.me?" me":""}`} style={{padding:"7px 10px"}}>
                  <div className={`lb-rank${i===0?" gold":i===1?" silver":i===2?" bronze":""}`}>{i===0?"🥇":i===1?"🥈":i===2?"🥉":`#${i+1}`}</div>
                  <div className="av" style={{width:24,height:24,fontSize:10}}>{l.av}</div>
                  <div style={{flex:1,fontSize:12,fontWeight:600}}>{l.n}{l.me&&<span className="tag tag-a" style={{marginLeft:5}}>You</span>}</div>
                  <div style={{fontSize:11,fontWeight:700,color:"var(--accent)"}}>{l.x.toLocaleString()}</div>
                </div>
              ))}
            </div>
          </div>
...
export default function App() {
  const [stage, setStage] = useState("loading"); // loading | auth | onboarding | app
  ...
  const SCREENS = {
    dashboard: <Dashboard profile={profile} setScreen={setScreen}/>,
    subjects:  <SubjectsScreen profile={profile}/>,
    quiz:      <QuizScreen profile={profile}/>,
    flashcards:<FlashcardsScreen/>,
    ai:        <AIScreen profile={profile}/>,
    planner:   <PlannerScreen profile={profile}/>,
    analytics: <AnalyticsScreen profile={profile}/>,
  };
        ...
        <div className="sidebar">
          ...
          <div className="sb-bottom">
            <div className="xp-row">
              <div className="xp-lbl"><span>Level 12 · 4,280 XP</span><span>5,000 XP</span></div>
              <div className="xp-track"><div className="xp-fill" style={{width:"86%"}}/></div>
            </div>
        ...
        <div className="main">
          <div className="topbar">
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div className="chip chip-fire">🔥 7 days</div>
              <div className="chip chip-xp">⚡ 4,280 XP</div>
